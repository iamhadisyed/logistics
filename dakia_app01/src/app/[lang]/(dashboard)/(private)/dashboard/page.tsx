    ...
    const loadStats = async () => {
        try {
            ...
            const [total, readyToPrint, delivered, inTransit] = await Promise.all([
                consignmentApi.list().catch(() => ({ data: { data: [] } })),
                consignmentApi.list({ status: CONSIGNMENT_STATUS.READY_TO_PRINT }).catch(() => ({ data: { data: [] } })),
                consignmentApi.list({ status: CONSIGNMENT_STATUS.DELIVERED }).catch(() => ({ data: { data: [] } })),
                consignmentApi.list({ status: CONSIGNMENT_STATUS.INTRANSIT }).catch(() => ({ data: { data: [] } })),
            ]);

            setStats({
                total: total.data.data?.length || 0,
                readyToPrint: readyToPrint.data.data?.length || 0,
                delivered: delivered.data.data?.length || 0,
                inTransit: inTransit.data.data?.length || 0,
            });
        } catch (error) {
                    ...
                    <Card>
                        <CardContent>
                            <Typography variant="h6" gutterBottom>
                                Welcome to Daakia Logistics
                            </Typography>
                            <Typography variant="body2" color="textSecondary">
                                Manage your consignments, services, and carriers from this dashboard.
                            </Typography>
                        </CardContent>
                    </Card>
                </Grid>
