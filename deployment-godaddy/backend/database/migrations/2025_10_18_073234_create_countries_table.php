...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('countries', function (Blueprint $table) {
            ...
            $table->enum('postcode_required', ['YES', 'NO'])->default('YES');
            $table->string('type', 1)->nullable();
            $table->string('region_collection', 45)->nullable();
            $table->unsignedInteger('numcode')->nullable()->comment('3 digit country iso code');
            $table->char('allow_express', 1)->nullable();
            $table->char('allow_classic', 1)->nullable();
            $table->char('eu_country', 1)->nullable();
            $table->string('shipping_advice', 255)->nullable();
            $table->enum('is_vatable', ['YES', 'NO'])->default('NO');
            $table->decimal('vat_rate', 2, 2)->nullable()->comment('VAT is always in percentage');
