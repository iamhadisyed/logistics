...
class CTTUntracked implements CarrierService {
    ...
    private function addWayBill(Consignment $consignment, $awb) {
        ...
        $this->pdf->Image("../images/cttlogo.png", 39, 1,58, 27);
        $this->pdf->setFont("helvetica", "", 6);
        $this->pdf->Image("../images/ctt_weight.jpg", 3, 64, 6, 5);
        $this->pdf->Image("../images/logo.png", 6, 2, 24, 10);
        $this->pdf->Text(9, 66, $consignment->getWeight());
        $this->pdf->setFont("helvetica", "", 8);
        $this->pdf->Text(6, 11, "Sender Details:");
        $this->pdf->setFont("helvetica", "", 7);
        
        //$this->pdf->Text(35, 3, "If undelivered");
        $this->pdf->Text(6, 16, $this->constants["CTT_SHIPPER_ADDRESSLINE1"]);
        $this->pdf->Text(6, 19, $this->constants["CTT_SHIPPER_ADDRESSLINE2"]);
        $this->pdf->Text(6, 22, $this->constants["CTT_SHIPPER_CITY"]);
        $this->pdf->Text(5.5, 25, $this->constants["CTT_SHIPPER_POSTCODE"]);
	...
	$this->pdf->Rotate(90, 5, 17);
        ...
        if (trim($consignment->getCity()) != '') {
            $this->pdf->Text($x, $y, trim(strtoupper($consignment->getCity())));
            $y += $inc;
        }
        ...
        $this->pdf->Image("../images/noscanner.png", 5, 75, 6, 6);
        $this->pdf->Text(3, 82, "No Scan");
        $this->pdf->Image("../images/nosignature.jpg", 5, 87, 6, 6);
        $this->pdf->Text(3, 94, "No Signature");
        
        $this->pdf->write1DBarcode($awb, 'C128', 22, 74, 70, 15, 0.4, $style, '');
        $this->pdf->setFont("freesans", "", 10);
        $arr = str_split($awb);	
        $this->pdf->Text(36, 92, $arr[0].$arr[1]. $arr[2]. " " . $arr[3].$arr[4].$arr[5].$arr[6].$arr[7].$arr[8]. " " . 
                                     $arr[9].$arr[10]);
