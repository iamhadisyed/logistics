...
class TCPDFBarcode {
	...
	public function setBarcode($code, $type) {
		switch (strtoupper($type)) {
        	...
        	case 'C128D': { // CODE 128 D
				$arrcode = $this->barcode_c128($code, 'D');
				break;
			}			
	...
	protected function barcode_c128($code, $type='') {
		...
		switch(strtoupper($type)) {
			...
			case 'D': { // MODE D
				...
				preg_match_all('/([0-9]{4,})/', substr($code, 2, strlen($code)) , $numseq, PREG_OFFSET_CAPTURE);
				if (isset($numseq[1]) AND !empty($numseq[1])) {
					...
					foreach ($numseq[1] as $val) {
						//echo "fffffffffffff" . $val[1];
						//die;
						$offset = 2;
						if ($offset > $end_offset) {
							// non numeric sequence
							//echo "($offset $end_offset aaaaaaa" . substr($code, $end_offset, ($offset - $end_offset));
							//die;
