...
class InputFileBulkPod {
    ...
    public function reset() {
        $this->if1_row = null;
        if (($this->if1_file_handle == null) || feof($this->if1_file_handle))
            return false;
    ...
    public function getDimensionArray() {
        ...
        if ($dimension_string != "") {
            ...
            foreach ($parcels as $parcel_str) {
                ...
                if (sizeof($new_dimensions) > 0)
                    $dimensions[0] = intval($new_dimensions[0]);
                if (sizeof($new_dimensions) > 1)
                    $dimensions[1] = intval($new_dimensions[1]);
                if (sizeof($new_dimensions) > 2)
                    $dimensions[2] = intval($new_dimensions[2]);
                if (sizeof($new_dimensions) > 3)
                    $dimensions[3] = intval($new_dimensions[3]);
                //
