...
class CSNotesFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        return CSNotes::getCsNotesListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(csn.id) AS total FROM cs_notes csn $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return CSNotes::getTotalNumberOfCsNotesFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM cs_notes  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return CSNotes::deleteCsNotesFromSql($sql);
    }
