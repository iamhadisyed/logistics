...
//echo $sql;die;
        t($sql, __METHOD__);
        return PermissionsLog::getPermissionsLogListFromSql($sql);
    }
    ...
    public function getAuditLog($Permissionsid,$limit = 15) {
        $sql = "SELECT pl.id, pl.userid, pl.logdate, pl.log_id, pl.log_type, pl.message, pl.previous_data, pl.current_data, u.user_name AS ipaddress FROM permissions_log AS pl ,`user` AS u WHERE pl.userid = u.id AND pl.log_id = '" . DbAccess3::escape($Permissionsid) . "' ORDER BY pl.logdate DESC limit ".$limit;
        t($sql, __METHOD__);
