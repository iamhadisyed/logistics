<?php

class ServiceLog extends DbAccess3 {

    /**
     * Construct
     *
     * @param id/array
     */
    public function __construct($mixedCreator = null) {

        $fieldList = array(
            'id' => 'number',
            'message' => 'string',
            'userid' => 'number',
            'logdate' => 'datetime',
            'ipaddress' => 'number',
            'userid' => 'number',
            'log_id' => 'number',
            'log_type' => 'string',
            'previous_data' => 'string',
            'current_data' => 'string'
        );
        //
        parent::__construct("service_log", 'id', $fieldList, $mixedCreator);
    }
    
        public static function logArray(){
        $displayArray   =   array(
            'name'      		=> 'name',
            'code'      		=> 'Service Code',
            'carrierid'   		=> 'Carrier',
            'accountnumber'   		=> 'Account Number',
            'type' 			=> 'Service Type',
            'servicecountry' 		=> 'Service Country',
            'fromweight' 		=> 'From Weight',
            'toweight' 		=> 'To Weight',
            'wieghttype' 		=> 'Weight Type',
            'supplier' 			=> 'Supplier',
            'servicetype'		=>'Service Type',
            'description'		=>'Description',
            'fuelsurchargecost'      	=> 'Fuel Surcharge Code',
            'fuelsurcharge'           	=> 'Fuel Surcharge',
            'fuelsurchargetype'      	=> 'Fuel Surcharge Type',
            'maxlength'               	=> 'Max Length',
            'maxwidth'                	=> 'Max Width',
            'maxheight'               	=> 'Max Height',
            'maxvolumetricweight'    	=> 'Max Volumetric Weight',
            'volumetricdenominator'    => 'Volumetric Denominator',
            'senddatacourier'		=> 'Send Data to Courier',
            'isdocument'		=> 'Is Document',
            'active'                   	=> 'Active',
            'deletedq'                 	=> 'Deleted',
            'uploadedcurrency'		=> 'Currency',
            'uploadedcurrencyvalue'	=> 'Currency Value',
            'registrationfee'		=> 'Registration Fee',
            'aditionalcharge'		=> 'Additional Charges',
            'origincountry'		=> 'Origin Country',
            'isuntrack'		=> 'Is Untrack',
            'accountowner'             => 'Account Owner',
            'remotearea'		=> 'Remote Area',
            'carrieraddresslimit'	=> 'Carrier Address Limit',
            'labelclassname'		=> 'Label Class Name',
            'transittime'		=> 'Transit Time',
            'requiredemail'            => 'Email Required',
            'requiredtelephone'        => 'Telephone Required',
            'shipmenttype'             => 'Shipment Type',
        );
        return $displayArray;
    }

    /**
     * Get list of Service Log objects, using sql given
     *
     * @param string $sql
     */
    public static function getServiceLogListFromSql($sql) {

        return DbAccess3::getListFromSql(__CLASS__, $sql);
    }

    /**
     * Get count of Service Log objects, using sql given
     *
     * @param string $sql
     */
    public static function getServiceLogCountFromSql($sql) {
        return DbAccess3::getSql(__CLASS__, $sql);
    }
    
     public function setUserid($val)
    {
        $this->valArray["userid"]=$val;
    }
     public function setIpaddress($val)
    {
        $this->valArray["ipaddress"]=$val;
    }
     public function setLogdate($val)
    {
        $this->valArray["logdate"]=$val;
    }
     public function setLogId($val)
    {
        $this->valArray["log_id"]=$val;
    }
     public function setLogType($val)
    {
        $this->valArray["log_type"]=$val;
    }
     public function setMessage($val)
    {
        $this->valArray["message"]=$val;
    }
     public function setPreviousData($val)
    {
        $this->valArray["previous_data"]=$val;
    }
     public function setCurrentData($val)
    {
        $this->valArray["current_data"]=$val;
    }
    
    public function createlog($user_id = '', $ipaddress, $Serviceid, $log_type, $message, $previous_data, $current_data) {
        if ($user_id == '') {
            $this->setUserId();
        } else {
            $this->setUserId($user_id);
        }
        $ipaddress = str_replace('.', '', $ipaddress);
        $this->SetLogdate(date('Y-m-d H:i:s'));
        $this->SetIpaddress($ipaddress);
        $this->SetLogId($Serviceid);
        $this->setLogType($log_type);
        $this->SetMessage($message);
        $this->setPreviousData($previous_data);
        $this->setCurrentData($current_data);
        $this->Save();
    }

}

// class