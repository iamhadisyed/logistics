<?php
// get settings
class ReconciliationDataFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return ReconciliationData::getReconciliationDataListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(rd.id) AS total FROM reconciliation_data rd $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ReconciliationData::getTotalNumberOfReconciliationDataFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM reconciliation_data  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ReconciliationData::deleteReconciliationDataFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE reconciliation_data $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ReconciliationData::updateReconciliationDataFromSql($sql);
    }
    ...
    public function addOrFilter($where, $operand = '=')
    {
        if (is_array($where)) {
            foreach ($where as $col => $val) {
                $this->orFilter[] = $col . " " . $operand . " '" . DbAccess3::escape($val) . "'";
