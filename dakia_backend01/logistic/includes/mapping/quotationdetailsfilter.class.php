...
class QuotationDetailsFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        return QuotationDetails::getQuotationDetailsListFromSql($sql);
    }
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(inv.id) AS total FROM quotation_details qd $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return QuotationDetails::getTotalNumberOfQuotationDetailsFromSql($sql);
    }
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM quotation_details  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return QuotationDetails::delete($sql);
    }
