...
class MarketPlaceOrderFilter {
    ...
    public function getList($debug = false) {
        ...
        $sql = "SELECT * FROM marketplace_order ord $where order by id desc LIMIT 5000";
    ...
    public function getColumnList($fields, $recordLimit = "", $debug = false) {
       ...
       // print_r(MarketPlaceOrder::getMarketPlaceOrderListFromSql($sql)); die;
        return MarketPlaceOrder::getMarketPlaceOrderListFromSql($sql);
    }
...
//        if ($this->order_by != "")
//          $sort = "ORDER BY " . $this->order_by;
        $sql = "SELECT count(ord.id) as total FROM marketplace_order `ord` $marketPlaceJoin $where";
//   echo $sql; die;
        t($sql, __METHOD__);
    ...
    public function getPagingList() {
        ...
        if ($this->order_by != "")
            $sort = " ORDER BY " . $this->order_by;
        $sql = "SELECT *, group_concat(title) as title, sum(item_price) as item_price FROM  marketplace_order ord $marketPlaceJoin  $where $groupBy $sort LIMIT $this->pageOffset , $this->rowsPerPage";
        //echo $sql; die;
        t($sql, __METHOD__);
        return MarketPlaceOrder::getMarketPlaceOrderListFromSql($sql);
    }
