...
class SkuBoxDetailFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return SkuBoxDetail::getSkuBoxDetailListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(so.id) AS total FROM sku_box_detail s $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return SkuBoxDetail::getTotalNumberOfSkuBoxDetailFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM sku_box_detail  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return SkuBoxDetail::deleteSkuBoxDetailFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE sku_box_detail $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return SkuBoxDetail::updateSkuBoxDetailFromSql($sql);
    }
