...
class CarrierDataFileLogFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                carrier_data_file_log cdfl $join                
                $where $orderBy";
        ...
        return CarrierDataFileLog::getCarrierDataFileLogListFromSql($sql);
    }
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(cdfl.id) AS total FROM carrier_data_file_log cdfl $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return CarrierDataFileLog::getTotalNumberOfCarrierDataFileLogFromSql($sql);
    }
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM carrier_data_file_log  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return CarrierDataFileLog::delete($sql);
    }
