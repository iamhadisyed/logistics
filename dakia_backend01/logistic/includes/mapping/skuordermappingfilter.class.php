...
class SkuOrderMappingFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return SkuOrderMapping::getSkuOrderMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(so.id) AS total FROM sku_order_mapping so $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return SkuOrderMapping::getTotalNumberOfSkuOrderMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM sku_order_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return SkuOrderMapping::deleteSkuOrderMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE sku_order $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return SkuOrder::updateSkuOrdersFromSql($sql);
    }
