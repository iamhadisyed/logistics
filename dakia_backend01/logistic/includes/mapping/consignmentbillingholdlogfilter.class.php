<?php 
/*
 * Consignment HOld Filter
 *
 */
class ConsignmentBillingHoldLogFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                consignment_billing_hold_log cbhl $join                
                $where $orderBy";
        ...
        return ConsignmentBillingHoldLog::getDataFromSql($sql);
    }
    public function getCount($soft = true) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(id) AS total FROM consignment_billing_hold_log cbhl $join $where";
        }
        return ConsignmentBillingHoldLog::getTotalNumberOfConsignmentBillingHoldLogFromSql($sql);
    }
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM consignment_billing_hold_log  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ConsignmentBillingHoldLog::deleteConsignmentBillingHoldLogFromSql($sql);
    }
