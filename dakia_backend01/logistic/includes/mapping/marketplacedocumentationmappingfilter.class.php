...
class MarketPlaceDocumentationMappingFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                market_place_documentation_mapping mpdm $join                
                $where $orderBy";
        ...
        return MarketPlaceDocumentationMapping::getMarketPlaceDocumentationMappingFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(mpdm.id) AS total FROM market_place_documentation_mapping mpdm $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return MarketPlaceDocumentationMapping::getTotalNumberOfMarketPlaceDocumentationMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM market_place_documentation_mapping mpdm  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return MarketPlaceDocumentationMapping::delete($sql);
    }
