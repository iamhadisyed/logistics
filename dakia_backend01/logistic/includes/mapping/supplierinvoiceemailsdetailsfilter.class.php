...
// get settings
class SupplierInvoiceEmailsDetailsFilter {
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                supplier_invoice_emails_details sied $join                
                $where $orderBy";
        ...
        return SupplierInvoiceEmailsDetails::getListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(sied.id) AS total FROM supplier_invoice_emails_details sied $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return SupplierInvoiceEmailsDetails::getTotalNumberFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM setting_supplier_email   $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return SupplierInvoiceEmailsDetails::deleteDataFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE setting_supplier_email  $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return SupplierInvoiceEmailsDetails::updateDataFromSql($sql);
    }
