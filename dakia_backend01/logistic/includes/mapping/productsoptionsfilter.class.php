...
class ProductsOptionsFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return ProductsOptions::getProductsOptionsListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(po.id) AS total FROM products_options po $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ProductsOptions::getTotalNumberOfProductsOptionsFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM products_options  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsOptions::deleteProductsOptionsFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE products_options $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsOptions::updateProductsOptionsFromSql($sql);
    }
