...
class PaymentsHistoryFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        return PaymentsHistory::getPaymentsHistoryListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(ph.id) AS total FROM payments_history ph $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return PaymentsHistory::getTotalNumberOfPaymentsHistoryFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM payments_history  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return PaymentsHistory::deletePaymentsHistoryFromSql($sql);
    }
    ...
    public static function customerTotalSpend($userAccountId = '') {
        $sql = "SELECT 
                    SUM(ph.`credit`) AS id
                  FROM
                    `payments_history` ph 
                  WHERE ph.`is_completed` = 'yes' 
                    AND ph.`account_id` IN ($userAccountId) ";
        return PaymentsHistory::getPaymentsHistoryListFromSql($sql);
    }
    
    public static function checkAccountTransaction($userAccountId = '') {
        $sql = "SELECT 
                SUM(ph.`debit`) AS id
               FROM
                 `payments_history` ph 
               WHERE ph.account_id IN ($userAccountId) ";
        
        $returnData = PaymentsHistory::getPaymentsHistoryListFromSql($sql);
        $isBool = false;
        if(count($returnData) > 0) {
            if (!empty($returnData[0]->getId())) {
                $isBool = true;
            }
        }
        return $isBool;
