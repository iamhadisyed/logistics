...
class BrandsFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return Brands::getBrandsListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(c.id) AS total FROM brands c $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return Brands::getTotalNumberOfBrandsFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM brands  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return Brands::deleteBrandsFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE brands $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return Brands::updateBrandFromSql($sql);
    }
