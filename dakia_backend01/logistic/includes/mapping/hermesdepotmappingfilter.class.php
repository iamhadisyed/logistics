<?php
// get settings
class HermesDepotMappingFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return HermesDepotMapping::getHermesDepotMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(so.id) AS total FROM hermes_depot_mapping h $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return HermesDepotMapping::getTotalNumberOfHermesDepotMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM hermes_depot_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return HermesDepotMapping::deleteHermesDepotMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE hermes_depot_mapping $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return HermesDepotMapping::updateHermesDepotMappingFromSql($sql);
    }
