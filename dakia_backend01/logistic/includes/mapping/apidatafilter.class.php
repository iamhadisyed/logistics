...
class ApiDataFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        return ApiData::getApiDataListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(ad.id) AS total FROM api_data ad $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ApiData::getTotalNumberOfApiDataFromSql($sql);
    }
    
    public function delete($debug = false) {
        $where = $this->getWhere();
        $sql = "DELETE FROM api_data $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ApiData::deleteApiDataDetailsFromSql($sql);
    }
