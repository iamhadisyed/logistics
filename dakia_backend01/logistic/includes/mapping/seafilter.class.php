...
class SeaFilter {
    ...
    /**
     * Get list of Sea items based on filter conditions
     *
     * @return array[Sea]
     */
    public function getPagingCount() {
        ...
        $sql = "SELECT count(pbm.id) as total FROM sea s  $checkJoin $where $groupBy $sort ";
        t($sql, __METHOD__);
        return Sea::getTotalNumberOfSeaFromSql($sql);
    }
    ...
    public function getParcelPagingCount() {
        ...
        $sql = "SELECT COUNT(id) AS total FROM (SELECT pbm.id FROM sea s  $checkJoin $where $groupBy $sort ) AS fdf";
        t($sql, __METHOD__);
        return Sea::getTotalNumberOfSeaFromSql($sql);
    }
    ...
    public function getPagingList($columns = '*') {
       ...
       $sql = "SELECT " . $columns . "  FROM sea s $checkJoin  $where $groupBy $sort LIMIT $this->pageOffset , $this->rowsPerPage";
       t($sql, __METHOD__);
        return Sea::getSeaListFromSql($sql);
    }
    ...
    function delete_parcel_from_mapping() {
        ...
        $sql =  "DELETE 
                    ...
                    $where";
        return Sea::deleteBySql($sql);
    }
    /**
     * Get count of Sea items based on filter conditions
     *
     * @return int
     */
    ...
    */
    ...
    public static function checkPreSortBag($bagId,$carrierHubId) {
        ...
        if($bagId > 0 && $carrierHubId > 0){
            ...
            $totalRec = Sea::getSeaListFromSql($sql);
            if(count($totalRec) == 1){
                $returnRes = true;
            }
        }
