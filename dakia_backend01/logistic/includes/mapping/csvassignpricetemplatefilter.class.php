<?php
// get settings
class CsvAssignPriceTemplateFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                csv_assign_price_template capt $join                
                $where $orderBy";
        ...
        return CsvAssignPriceTemplate::getCsvAssignPirceTemplateListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(capt.id) AS total FROM csv_assign_price_template capt $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return CsvAssignPriceTemplate::getTotalNumberOfCsvAssignPirceTemplateFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM csv_assign_price_template $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return CsvAssignPriceTemplate::deleteCsvAssignPirceTemplateFromSql($sql);
    }
