...
class UserAuditFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return UserAudit::getUserAuditFromSql($sql);
    }

    public function getCount($soft=false,$debug = false)
    {
        $soft = false;
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(ua.id) AS total FROM user_audit ua $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return UserAudit::getTotalNumberOfUserAuditFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM user_audit  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return UserAudit::delete($sql);
    }
    ...
    public function update($data, $debug = false)
    {
        ...
        $set = rtrim($set, ", ");
        $where = $this->getWhere();
        $sql = "UPDATE user_audit SET $set {$where}";
        if ($debug) {
            echo $sql;
            die();
        }
