 ...
 *
 */
class ConsignmentBillingHoldFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                consignment_billing_hold cbh $join                
                $where $orderBy";
        ...
        return ConsignmentBillingHold::getDataFromSql($sql);
    }
    public function getCount($soft = true) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(id) AS total FROM consignment_billing_hold cbh $join $where";
        }
        return ConsignmentBillingHold::getTotalNumberOfConsignmentBillingHoldFromSql($sql);
    }
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM consignment_billing_hold  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ConsignmentBillingHold::deleteConsignmentBillingHoldFromSql($sql);
    }
