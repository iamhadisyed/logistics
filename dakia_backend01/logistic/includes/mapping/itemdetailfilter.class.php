...
class ItemDetailFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return ItemDetail::getItemDetailListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(i.id) AS total FROM item_details i $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ItemDetail::getTotalNumberOfItemDetailFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM item_detail  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ItemDetail::deleteItemDetailFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE item_detail $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ItemDetail::updateItemDetailFromSql($sql);
    }
