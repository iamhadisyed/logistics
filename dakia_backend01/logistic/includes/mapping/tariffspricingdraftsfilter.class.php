<?php
// get settings
class TariffsPricingDraftsFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                 tariffs_pricing_drafts tpd $join                
                $where $orderBy";
        ...
        return TariffsPricingDrafts::getTariffsPricingDraftsListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(tpd.id) AS total FROM tariffs_pricing_drafts tpd $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return TariffsPricingDrafts::getTotalNumberOfTariffsPricingDraftsFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM tariffs_pricing_drafts  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return TariffsPricingDrafts::deleteTariffsPricingDraftsFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE tariffs_pricing_drafts $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return TariffsPricingDrafts::updateTariffsPricingDraftsFromSql($sql);
    }
