<?php
// get settings
class CountryZonesMappingFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                country_zones_mapping czm $join                
                $where $orderBy";
        ...
        return CountryZonesMapping::getCountryZonesMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(czm.id) AS total FROM country_zones_mapping czm $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return CountryZonesMapping::getTotalNumberOfCountryZonesMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM country_zones_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return CountryZonesMapping::deleteCountryZonesMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE country_zones_mapping $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return CountryZonesMapping::updateCountryZonesMappingFromSql($sql);
    }
