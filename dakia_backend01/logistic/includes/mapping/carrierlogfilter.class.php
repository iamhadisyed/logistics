...
//echo $sql;die;
        t($sql, __METHOD__);
        return CarrierLog::getCarrierLogListFromSql($sql);
    }
    ...
    public function getAuditLog($ServiceLogid,$limit=15) {
        $sql = "SELECT sl.id, sl.userid, sl.logdate, sl.log_id, sl.log_type, sl.message, sl.previous_data, sl.current_data, u.user_name AS ipaddress FROM carrier_log AS sl ,`user` AS u WHERE sl.userid = u.id AND sl.log_id = '" . DbAccess3::escape($ServiceLogid) . "' ORDER BY sl.logdate DESC limit $limit";
        t($sql, __METHOD__);
