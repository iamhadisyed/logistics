...
/*
 * RemoteAreaChargesTarriff Filter
 *
 */


class RemoteAreaChargesTariffsFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                remotearea_charges_tariffs rct $join                
        ...
        return RemoteAreaChargesTariffs::getRemoteareaChargesTariffListFromSql($sql);
    }
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(inv.id) AS total FROM remotearea_charges_tariffs rct $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return RemoteAreaChargesTariffs::getTotalNumberOfRemoteareaChargesTariffFromSql($sql);
    }
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM remotearea_charges_tariffs  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return RemoteAreaChargesTariffs::delete($sql);
    }
