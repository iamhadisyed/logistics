...
class TariffAdditionalChargesFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                tariff_additional_charges tac $join                
        ...
        return TariffAdditionalCharges::getTariffAdditionalChargesListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(uasc.id) AS total FROM  Table: tariff_additional_charges tac $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return TariffAdditionalCharges::getTotalNumberOfTariffAdditionalChargesFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM  tariff_additional_charges  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return TariffAdditionalCharges::deleteTariffAdditionalChargesFromSql($sql);
    }
