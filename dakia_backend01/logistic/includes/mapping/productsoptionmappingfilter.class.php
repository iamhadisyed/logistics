<?php
// get settings
class ProductsOptionMappingFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                products_option_mapping pom $join                
                $where $orderBy";
        ...
        return ProductsOptionMapping::getProductsOptionMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(pom.id) AS total FROM products_option_mapping pom $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ProductsOptionMapping::getTotalNumberOfProductsOptionMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM products_option_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsOptionMapping::deleteProductsOptionMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE products_option_mapping $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsOptionMapping::updateProductsOptionMappingFromSql($sql);
    }
