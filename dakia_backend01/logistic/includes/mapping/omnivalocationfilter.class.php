...
////////////////////////////////////////////////////
//
// Class for dealing with Omniva Location for Parcel Machine
//
////////////////////////////////////////////////////

/**
 * Omniva Location filter class
 * @package News Releases
 */

class OmnivaLocationFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return OmnivaLocation::getOmnivaLocationListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(s.id) AS total FROM omniva_location o $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return OmnivaLocation::getTotalNumberOfOmnivaLocationFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM omniva_location  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return OmnivaLocation::deleteOmnivaLocationFromSql($sql);
    }
