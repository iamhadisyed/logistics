...
class InvoicesManualDetailsFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                invoices_manual_details imd $join                
                $where $orderBy";
        ...
        return InvoicesManualDetails::getManualInvoiceDetailsListFromSql($sql);
    }
    public function getCount($soft = true) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(id) AS total FROM invoices_manual_details imd $join $where";
        }
        return InvoicesManualDetails::getTotalNumberOfManualInvoiceDetailsFromSql($sql);
    }
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM invoices_manual_details  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return InvoicesManualDetails::deleteManualInvoiceDetailsDetailsFromSql($sql);
    }
