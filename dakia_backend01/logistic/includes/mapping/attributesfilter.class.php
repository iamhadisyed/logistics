...
class AttributesFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return Attributes::getAttributesListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(a.id) AS total FROM attributes a $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return Attributes::getTotalNumberOfAttributesFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM attributes  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return Attributes::deleteAttributesFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE attributes $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return Attributes::updateAttributesFromSql($sql);
    }
    ...
    public function getAllCategories() {
        $sql = "SELECT 
                ...
                FROM
                  ...
                  JOIN `categories` hi 
                    ON hi.id = ho.id ";
        return Categories::getCategoriesListFromSql($sql);
    }
