...
class QuotationPaymentsHistoryFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                quotation_payments_history qph $join                
                $where $orderBy";
        ...
        return QuotationPaymentsHistory::getQuotationPaymentsHistoryListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(qph.id) AS total FROM quotation_payments_history qph $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return QuotationPaymentsHistory::getTotalNumberOfQuotationPaymentsHistoryFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM quotation_payments_history  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return QuotationPaymentsHistory::deleteQuotationPaymentsHistoryFromSql($sql);
    }
