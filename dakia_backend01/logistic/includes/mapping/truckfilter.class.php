...
class TruckFilter {
    ...
    /**
     * Get list of Truck items based on filter conditions
     *
     * @return array[Truck]
     */
    public function getPagingCount() {
        ...
        $sql = "SELECT count(pbm.id) as total FROM truck t  $checkJoin $where $groupBy $sort ";
        t($sql, __METHOD__);
        return Truck::getTotalNumberOfTruckFromSql($sql);
    }
    ...
    public function getParcelPagingCount() {
        ...
        $sql = "SELECT COUNT(id) AS total FROM (SELECT pbm.id FROM truck t  $checkJoin $where $groupBy $sort ) AS fdf";
        t($sql, __METHOD__);
        return Truck::getTotalNumberOfTruckFromSql($sql);
    }
    ...
    public function getPagingList($columns = '*') {
       ...
       $sql = "SELECT " . $columns . "  FROM truck t $checkJoin  $where $groupBy $sort LIMIT $this->pageOffset , $this->rowsPerPage";
       t($sql, __METHOD__);
        return Truck::getTruckListFromSql($sql);
    }
    ...
    function delete_parcel_from_mapping() {
        ...
        $sql =  "DELETE 
                    ...
                    $where";
        return Truck::deleteBySql($sql);
    }
    /**
     * Get count of Truck items based on filter conditions
     *
     * @return int
     */
    ...
    */
    ...
    public static function checkPreSortBag($bagId,$carrierHubId) {
        ...
        if($bagId > 0 && $carrierHubId > 0){
            ...
            $totalRec = Truck::getTruckListFromSql($sql);
            if(count($totalRec) == 1){
                $returnRes = true;
            }
        }
