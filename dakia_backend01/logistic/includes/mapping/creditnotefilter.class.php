...
class CreditNoteFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        return CreditNote::getCreditNoteListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(cn.id) AS total FROM credit_note cn $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return CreditNote::getTotalNumberOfCreditNoteFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM credit_note  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return CreditNote::deleteCreditNoteFromSql($sql);
    }
