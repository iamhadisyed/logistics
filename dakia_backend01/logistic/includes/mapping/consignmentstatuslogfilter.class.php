...
class ConsignmentStatusLogFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                consignment_status_log csl $join                
        ...
        return ConsignmentStatusLog::getConsignmentStatusLogListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(csl.id) AS total FROM  consignment_status_log csl $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ConsignmentStatusLog::getTotalNumberOfConsignmentStatusLogFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM consignment_status_log  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ConsignmentStatusLog::deleteConsignmentStatusLogFromSql($sql);
    }
