...
//echo $sql;die;
    ...
    public function getAuditLog($ServiceLogid) {
        $sql = "SELECT sl.id, sl.userid, sl.logdate, sl.log_id, sl.log_type, sl.message, sl.previous_data, sl.current_data, u.user_name AS ipaddress FROM service_log AS sl ,`user` AS u WHERE sl.userid = u.id AND sl.log_id = '" . DbAccess3::escape($ServiceLogid) . "' ORDER BY sl.logdate ASC";
        t($sql, __METHOD__);
