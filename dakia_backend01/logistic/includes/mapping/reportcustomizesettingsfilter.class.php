...
/*
 * ReportCustomizeSettings Filter
 *
 */

class ReportCustomizeSettingsFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                report_customize_settings rcs $join                
                $where $orderBy";
        ...
        return ReportCustomizeSettings::getReportCustomizeSettingsListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(rcs.id) AS total FROM report_customize_settings rcs $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ReportCustomizeSettings::getTotalNumberOfReportCustomizeSettingsFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM report_customize_settings  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ReportCustomizeSettings::deleteReportCustomizeSettingsFromSql($sql);
    }
