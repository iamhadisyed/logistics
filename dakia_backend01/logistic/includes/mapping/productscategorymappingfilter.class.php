<?php
// get settings
class ProductsCategoryMappingFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                products_category_mapping pcm $join                
                $where $orderBy";
        ...
        return ProductsCategoryMapping::getProductsCategoryMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(pcm.id) AS total FROM products_category_mapping p $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ProductsCategoryMapping::getTotalNumberOfProductsCategoryMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM products_category_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsCategoryMapping::deleteProductsCategoryMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE products_category_mapping $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsCategoryMapping::updateProductsCategoryMappingFromSql($sql);
    }
