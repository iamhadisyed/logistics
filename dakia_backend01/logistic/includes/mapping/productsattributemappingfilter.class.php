<?php
// get settings
class ProductsAttributeMappingFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                products_attribute_mapping pam $join                
                $where $orderBy";
        ...
        return ProductsAttributeMapping::getProductsAttributeMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(pam.id) AS total FROM products_attribute_mapping pam $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ProductsAttributeMapping::getTotalNumberOfProductsAttributeMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM products_attribute_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsAttributeMapping::deleteProductsAttributeMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE products_attribute_mapping $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsAttributeMapping::updateProductsAttributeMappingFromSql($sql);
    }

    public function set($set) {
        if (is_array($set)) {
            foreach ($set as $col => $val) {
                $this->setUpdateValue[] = $col . "='" . DbAccess3::escape($val) . "'";
            }
        }
    }
    ...
    {
        $set = "";
        if (!empty($this->setUpdateValue)) {
            $set = implode(" , ", $this->setUpdateValue);
        }
        if (!empty($set)) {
            $set = " SET " . $set;
        }
