<?php 
/*
 * UserAccountServiceCharges Filter
 *
 */

class UserAccountServiceChargesFilter {
    ...
    public function getList($columns = '*', $debug = false) {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                user_account_service_charges uasc $join                
                $where $orderBy";
        ...
        return UserAccountServiceCharges::getUserAccountServiceChargesListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false) {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(uasc.id) AS total FROM  Table: user_account_service_charges uasc $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return UserAccountServiceCharges::getTotalNumberOfUserAccountServiceChargesFromSql($sql);
    }
    ...
    public function delete($debug = false) {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM  user_account_service_charges  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return UserAccountServiceCharges::deleteUserAccountServiceChargesFromSql($sql);
    }
