<?php
// get settings
class ProductsImageMappingFilter
{
    private $filter = [];
    private $orFilter = [];
    private $orderBy = [];
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        $sql = "SELECT SQL_CALC_FOUND_ROWS " . $columns . " FROM
                products_image_mapping pim $join                
                $where $orderBy";
        ...
        return ProductsImageMapping::getProductsImageMappingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(pim.id) AS total FROM products_image_mapping pim $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ProductsImageMapping::getTotalNumberOfProductsImageMappingFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM products_image_mapping  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsImageMapping::deleteProductsImageMappingFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE products_image_mapping $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ProductsImageMapping::updateProductsImageMappingFromSql($sql);
    }
