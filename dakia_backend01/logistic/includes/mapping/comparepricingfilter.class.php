...
class ComparePricingFilter
{
    ...
    public function getList($columns = '*', $debug = false)
    {
        ...
        return ComparePricing::getPricingListFromSql($sql);
    }
    ...
    public function getCount($soft = true, $debug = false)
    {
        if ($soft) {
            ...
            $where = $this->getWhere();
            $sql = "SELECT COUNT(c.id) AS total FROM compare_pricings c $join $where";
            if ($debug) {
                echo $sql;
                die();
            }
        }
        return ComparePricing::getTotalNumberOfPricingsFromSql($sql);
    }
    ...
    public function delete($debug = false)
    {
        ...
        $where = $this->getWhere();
        $sql = "DELETE FROM compare_pricings  $join $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ComparePricing::deletePricingsFromSql($sql);
    }

    public function update($debug = false){
        $where = $this->getWhere();
        $set = $this->getSet();
        $sql = "UPDATE compare_pricings $set $where";
        if ($debug) {
            echo $sql;
            die();
        }
        return ComparePricing::updatePricingsFromSql($sql);
    }
