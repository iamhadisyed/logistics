...
use Auth\customGrantType\Controller\SmartTokenController;
use Auth\customGrantType\SmartCredentials;
    ...
    UserInfoControllerInterface
{
    ...
    protected $storageMap = array(
        ...
        'smarttrack_app'    => 'Auth\customGrantType\SmartCredentialsInterface',
        ...
        if (is_null($this->clientAssertionType)) {
            ...
            foreach ($this->grantTypes as $grantType) {
                if (!$grantType instanceof ClientAssertionTypeInterface) {
                   ...
                   if($grantType instanceof SmartAssertionTypeInterface){
	                   $this->clientAssertionType = new HttpSmartBasic($this->storages['client_credentials'], $config);
	                   $isSmartAppReq=true;
	                   break;
                   }else{
...
//                    break;
	...
	if($isSmartAppReq){
		return new SmartTokenController($accessTokenResponseType, $this->storages['client'], $this->grantTypes, $this->clientAssertionType, $this->getScopeUtil());
	}else{
    ...
    protected function getDefaultGrantTypes()
    {
	    ...
	    if (isset($this->storages['smarttrack_app'])) {
		    $config = array_intersect_key($this->config, array('allow_credentials_in_request_body' => ''));
		    $grantTypes['smarttrack_app'] = new SmartCredentials($this->storages['smarttrack_app'], $config);
	    }
