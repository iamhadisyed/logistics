...
namespace Auth\customGrantType\Controller;

use Auth\customGrantType\ClientAssertionType\SmartAssertionTypeInterface;
 ...
 */
class SmartTokenController implements TokenControllerInterface
{
    /**
     ...
     */
    public function __construct(AccessTokenInterface $accessToken, ClientInterface $clientStorage, array $grantTypes = array(), SmartAssertionTypeInterface $clientAssertionType = null, ScopeInterface $scopeUtil = null)
    {
    ...
    {
//	    $response->addParameters(['status'=>'error']);
...
//	    if (!is_null($request->request('plat_form'))) {
		    if(!$this->clientStorage->verifyPlatFrom($request->request('client_id'),$request->request('client_secret'),"smarttrack_app")){
			    $response->setError(400, 'invalid_client', 'The client credentials are invalid. Please check your platform value');
			    return false;
		    }

//	    }else{
//		    $response->setError(400, 'missing_credentials', 'Platform field is missing. Please check your platform value');
//		    return false;
//	    }
