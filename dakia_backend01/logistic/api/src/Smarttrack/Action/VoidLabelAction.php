...
use SmarttrackTransformer\VoidLabelTransformer;
use Zend\InputFilter\Factory as InputFilterFactory;

class VoidLabelAction {
...
//	    echo "<pre>"; print_r($apiUserData); echo "</pre>"; die();
        ...
        $validateScannigData = $this->validate($paramData, ['hawb']);
        $scanningResp = ApiFunctions::voidLabel($validateScannigData, $apiUserData);
        $transformer = new VoidLabelTransformer();
        $hal = $transformer->transformCollection($scanningResp);
        $resultData = $this->renderer->render($request, $response, $hal);
        ApiFunctions::AddApiLog("VoidLabels", $paramData,$resultData ,$userId);
        return $resultData;
    }
