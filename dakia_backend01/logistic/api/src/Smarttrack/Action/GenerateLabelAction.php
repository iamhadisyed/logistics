...
use SmarttrackTransformer\GenerateLabelTransformer;
...
//        $validateData = $this->validate($shipData, ['sender_contact','sender_address_line_1','sender_city','sender_postcode','sender_country_iso','receiver_contact','receiver_address_line_1','receiver_city','receiver_postcode','receiver_country_iso','service_code','weight','description','uuid']);
        ...
        if(count($errors)) {
            ...
            $hal = $transformer->transformCollection($generateLabelRes);
            $resultData = $this->renderer->render($request, $response, $hal);
            ApiFunctions::AddApiLog("check-driver-assign", $shipData,$resultData ,$userId);
            return $resultData;*/
        } else {
...
//            $transformer = new GenerateLabelTransformer();
//            $hal = $transformer->transformCollection($generateLabelRes);
//            $resultData = $this->renderer->render($request, $response, $hal);
//            ApiFunctions::AddApiLog("CreateLabel", $shipData,$resultData ,$userId);
//            return $resultData;
        }
        $transformer = new GenerateLabelTransformer();
        $hal = $transformer->transformCollection($generateLabelRes);
        $resultData = $this->renderer->render($request, $response, $hal);
        ApiFunctions::AddApiLog("GenerateLabel", $shipData,$resultData ,$userId);
        return $resultData;
    }
    /**
