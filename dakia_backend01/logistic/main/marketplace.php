...
class Page extends BasePage {
    ...
    protected function init() {
        ...
        else if (isset($this->form_vars["func"]) && $this->form_vars["func"] == "saveManualRecord") 
        {
            ...
            for($counter = 0; $counter< $this->form_vars["elindex-hardcode"]; $counter++)
            {
                if($this->form_vars["sku"][$counter] == '')
                {
                    $output['MESSAGE'][] = "Please enter ".$counter." SKU";
                }
                else
                {
                    $sku[] = $this->form_vars["sku"][$counter];
                }
            
                if($this->form_vars["quantity"][$counter] == '')
                {
                    $output['MESSAGE'][] = "Please enter Quantity";
                }
                else
                {
                    $quantity[] = $this->form_vars["quantity"][$counter];
                }
            
                if($this->form_vars["price"][$counter] == '')
                {
                    $output['MESSAGE'][] = "Please enter Price.";
                }
                else
                {
                    $price[] = $this->form_vars["price"][$counter];
                }
            
                if($this->form_vars["title"][$counter] == '')
                {
                    $output['MESSAGE'][] = "Please enter Title.";
                }
                else
                {
                    $title[] = $this->form_vars["title"][$counter];
                }
            }
            ...
            $marketPlaceOrderFilter = new MarketPlaceOrderFilter();
            $marketPlaceOrderFilter->addFilter("    marketplace_order_number = '".$manualOrderNumber."'");
            $isExist = $marketPlaceOrderFilter->getList();
            
            if(count($isExist) > 0)
            {
                $marketPlaceOrder = new MarketPlaceOrder($isExist[0]->getId());
            }
            else
            {
            ...
            $marketPlaceOrderDetailsFilter = new MarketPlaceOrderDetailsFilter();
            $marketPlaceOrderDetailsFilter->addFilter("    marketplace_order_id = '".$marketPlaceOrderTableId."'");
            $isExistDetail = $marketPlaceOrderDetailsFilter->getList();
            
            //die;
            for($counter = 0; $counter< $this->form_vars["elindex-hardcode"]; $counter++)
            {
                if(count($isExistDetail) >0 && $counter < count($isExistDetail))
                {
                    $marketPlaceOrderDetails = new MarketPlaceOrderDetails($isExistDetail[$counter]->getId());
                }
                else
                {
                ...
                $marketPlaceOrderDetails->setSku($sku[$counter]);
                $marketPlaceOrderDetails->setTitle($title[$counter]);
                $marketPlaceOrderDetails->setQuantityPurchased($quantity[$counter]);
                $marketPlaceOrderDetails->setItemPrice($price[$counter]);
            ...
            $hscode = $this->form_vars["hscode"];
            $eori = $this->form_vars["eori"];
            ...
            $consignmentArray['eori_number'] = $eori;
            ...
            for ($count = 0; $count < $numberOfPieces; $count++) { //for loop change .. upto number of pieces
                ...
                $consignmentArray['parcel'][$count]['items'][$count]['item_description'] = $description;
                $consignmentArray['parcel'][$count]['items'][$count]['item_url'] = '';
                $consignmentArray['parcel'][$count]['items'][$count]['item_sku'] = $sku;
                $consignmentArray['parcel'][$count]['items'][$count]['no_of_items'] = $numberOfPieces;
                $consignmentArray['parcel'][$count]['items'][$count]['item_value'] = $value;
                $consignmentArray['parcel'][$count]['items'][$count]['weight'] = $weight;
                $consignmentArray['parcel'][$count]['items'][$count]['tarrif_no'] = '';
                $consignmentArray['parcel'][$count]['items'][$count]['hscode'] = $hscode;
                $consignmentArray['parcel'][$count]['items'][$count]['manufacture_country_iso'] = "GB";
            }
            ...
            foreach ($bulkIds as $Ids) {
                ...
                for ($count = 0; $count < $numberOfPieces; $count++) { //for loop change .. upto number of pieces
                    ...
                    $consignmentArray['parcel'][$count]['items'][$count]['item_description'] = $description;
                    $consignmentArray['parcel'][$count]['items'][$count]['item_url'] = '';
                    $consignmentArray['parcel'][$count]['items'][$count]['item_sku'] = $marketOrderDetails->getSku();
                    $consignmentArray['parcel'][$count]['items'][$count]['no_of_items'] = $noOfPieces;
                    $consignmentArray['parcel'][$count]['items'][$count]['item_value'] = $marketOrderDetails->getItemPrice();
                    $consignmentArray['parcel'][$count]['items'][$count]['weight'] = $weight;
                    $consignmentArray['parcel'][$count]['items'][$count]['tarrif_no'] = '';
                    $consignmentArray['parcel'][$count]['items'][$count]['hscode'] = $description;
                    $consignmentArray['parcel'][$count]['items'][$count]['manufacture_country_iso'] = $description;                    
                }
            ...
            if ($consignmentId > 0) {
                ...
                $output .= '</select>                                          
                                           ...
                                           <input type="text" class="form-control form-filter" name="hscode" id= "hscode">
                                           </div>
                                           <div class="col-md-4">
                                           <strong>EORI:</strong>
                                           <input type="text" class="form-control form-filter" name="eori" id= "eori">
                                           </div>';
            }
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $(document).on('click', '.add_more_sku_order_keys', function () {
                    ...
                    var currency = $(clone).find('.currency .bs-select').attr('name');
                    var currencyId = $(clone).find('.currency .bs-select').attr('id');
                    $(this).remove();
                    ...
                    $(clone).find('.currency .bs-select').attr('name', currency.replace(/\d+/, elindex));
                    $(clone).find('.currency .bs-select').attr('id', currencyId.replace(/\d+/, elindex));
                    $(clone).find('.bootstrap-select.currency').replaceWith(function () {
                        return $('#currency_' + elindex, this);
                    });
                    $(clone).find('#currency_' + elindex).selectpicker('refresh');
                ...
                $(document).on('click', '.remove_sku_order_key', function () {
                    ...
                    swal({
                        ...
                        closeOnCancel: true
                            ...
                            function (isConfirm) {
                                if (isConfirm) {
                                    if ($('.manual_order .remove_sku_order_key').length > 1) {
                                        ...
                                        if ($('.add_more_sku_order_keys').length == 0) {
                                            ...
                                            $('.manual_order .remove_sku_order_key').last().parent().prepend(addMore);
                                        }
                                    } else {
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="has-float-label input-icon right">
                                            ...
                                            echo Ddl::generateArrayDDL('currency', $arrayCurrency, '', '', ' class="form-control bs-select currency" rel="tooltip" data-original-title="Currency" placeholder="Currency"','Select Currency',"currency_0");?>
                                        </div> 
                                    </div>
