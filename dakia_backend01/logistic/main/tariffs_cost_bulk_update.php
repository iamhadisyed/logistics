...
class Page extends BasePage
{
	...
	public function init()
	{
		...
		if (trim(util_request("action")) == "BULK_UPLOAD_TARIFF") 
		{
			...
			if($id > 0)
			{
				$tariffData			=	new CostTariffsFilter();
				$tariffData->addFieldFilter('id', $id	);
				
				$recordSet	=	$tariffData->getList();
				if(count($recordSet)> 0)
				{
					$dataTariff	=	$recordSet[0];
					if(trim($option) == 'tariff_cost')
						$dataTariff->setTariffCost($value);
					else if(trim($option) == 'unit_cost')	
						$dataTariff->setUnitCost($value);	
	...
	function updatetariffvalue(tariffId, tariffValue, tariffOption)
	{
		...
		$.post("tariffs_cost_bulk_update.php", {action:'BULK_UPLOAD_TARIFF',id:tariffId,  value:tariffValue, option:tariffOption}, function(data) {
