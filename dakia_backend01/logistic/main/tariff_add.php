...
class Page extends BasePage
{
    ...
    protected function init()
    {
        ...
        if (isset($this->form_vars['action']) && $this->form_vars['action'] == 'save_tariff') {
            ...
            $templateType = $this->form_vars['template_type'];
			$type = $this->form_vars['chkType'];
            ...
            $checkSupplierAgentTariff = false;
            if($status == 1 && $type == 'supplier') {
//                $checkActiveTariff = Tariffs::checkSupplierTariffActive($user_account_id,$service_id,$start_date,$end_date);
//                if($checkActiveTariff['tariff_found'] > 0) {
//                    $activeTariffIds = explode( ',',$checkActiveTariff['ids']);
//                    if(count($activeTariffIds)) {
//                        foreach($activeTariffIds as $activeTariffId) {
//                            $tariffs = new Tariffs($activeTariffId);
//                            $tariffs->setStatus(0);
//                            $tariffs->save();
//                        }
//                    }
//                }
                ...
                $tariffsDetailsFilter->addFieldFilter('     tariffs_id', $tariffId);
                $tariffsDetailsFilter->addFilter('       agent_id IS NOT NULL ');
                $tariffsDetailsFilterObjs = $tariffsDetailsFilter->getList();
                if(count($tariffsDetailsFilterObjs)) {
                    $checkSupplierAgentTariff = true;
                }
            }
            $date_added = time();
...
// upload tariff_file with save page button
             if(isset($_FILES['tariff_file'])){
                @$tariff_file = $_FILES['tariff_file'];
                    if (!empty($tariff_file['name'])) {
                        $file_name = $tariff_file['name'];
                        $path_parts = pathinfo($file_name);
                        $ext = strtolower($path_parts['extension']);
                        $basename = $path_parts['basename'];
                        if ($ext == 'csv' || $ext == 'xlsx' || $ext == 'docx' || $ext == 'doc' || $ext == 'pdf') {
                            $new_file_name = "tariff_" . $added_by . "_" . $basename;
                            $relPath = '../_assets/tariff_files/'.$new_file_name;
                            if (!file_exists("../_assets/tariff_files/")){
                            @mkdir("../_assets/tariff_files/", 0775);
                            }    
                            move_uploaded_file($tariff_file['tmp_name'], $relPath);
                            $tariffsObj->setTariffFile($new_file_name);
                    }
                }
            }
            ...
            $tariffZoneType = $this->form_vars['tariff_zone_type'];
            $oldTariffDetails = new TariffsDetailsFilter();
            $oldTariffDetails->addFieldFilter('     tariffs_id', $tarif_id);
            $oldTariffDetails = $oldTariffDetails->getList();
            $tariff_details_data_old = [];
            if($checkSupplierAgentTariff) {
                carrierServiceCustomizeRules::deleteByUserAccountIdAndServiceId($user_account_id,$service_id );
            }
            foreach ($oldTariffDetails as $tarrifDetail) {
                $fromZone = new CarrierZones($tarrifDetail->getFromZoneId());
                $toZone = new CarrierZones($tarrifDetail->getToZoneId());
                $weightFrom = $tarrifDetail->getWeightFrom();
                $weightTo = $tarrifDetail->getWeightTo();
                $tariff_details_data_old[] = [
                    'from_zone' => $fromZone->getName(),
                    'to_zone' => $toZone->getName(),
                    'weight_from' => $weightFrom,
                    'weight_to' => $weightTo,
                    'weight_cost' => $tarrifDetail->getWeightCost(),
                    'piece_cost' => $tarrifDetail->getPieceCost(),
                    'formula' => $tarrifDetail->getFormula()
                ];
                if($checkSupplierAgentTariff) {
                    $agentId = '';
                    $tariffsDetailsFilter = new TariffsDetailsFilter();
                    $tariffsDetailsFilter->addFieldFilter('       tariffs_id', $tarif_id);
                    $tariffsDetailsFilter->addFieldFilter('       weight_from', $weightFrom);
                    $tariffsDetailsFilter->addFieldFilter('       weight_to', $weightTo);
                    $tariffsDetailsFilterObjs = $tariffsDetailsFilter->getList();
                    if(count($tariffsDetailsFilterObjs)) {
                        $agentId = $tariffsDetailsFilterObjs[0]->getAgentId();
                    }
                    if($agentId != "" && $agentId > 0) {
                        $carrierServiceCustomizeRules = new carrierServiceCustomizeRules();
                        $carrierServiceCustomizeRules->setServiceid($service_id);
                        $carrierServiceCustomizeRules->setAgentid($agentId);
                        $carrierServiceCustomizeRules->setUserAccountId($user_account_id);
                        $carrierServiceCustomizeRules->setFromWeight($weightFrom);
                        $carrierServiceCustomizeRules->setToWeight($weightTo);
            ...
            if ($checkDetailsBoxSelect > 0) {
                foreach ($this->form_vars['tariffs'] as $tf) {
                    if($tariffZoneType == "single") {
                        $from_zone_id = $this->form_vars['from_zone_id'];
                    } else {
                    ...
                    if($templateType == "kg") {
                        $user_peice_cost = 0.00;
                    }
                    if($templateType == "item") {
                        $user_weight_cost = 0.00;
                    }
                    $formula = $tf['formula'];
                    ...
                    $tariff_details_data[] = [
                        ...
                        'weight_to' => $weight_to,
                        'weight_cost' => $user_weight_cost,
                        'piece_cost' => $user_peice_cost,
                        'formula' => $formula
                    ];
                }
            } else {
                ...
                if($tariffZoneType == "single") {
                    foreach ($this->form_vars['weight'] as $w) {
                        $weight = explode('/', $w);
                        $w_from = '';
                        $w_to = '';
                        if (count($weight) > 0) {
                            $w_from = $weight[0];
                            $w_to = $weight[1];
                            foreach ($this->form_vars['zones'][$w] as $current_z => $z) {
                                $zones = explode('/', $z);
                                $z_price_weight_cost = '';
                                $z_price_piece_cost = '';
                                if (count($zones) > 0) {
                                    $z_price_weight_cost = $zones[0];
                                    $z_price_piece_cost = (isset($zones[1]) ? $zones[1] : 0.00);
                                }
                                $current_zone_to = 0;
                                if (is_numeric($this->form_vars['tariff_id']) && $this->form_vars['tariff_id'] > 0 && is_numeric($current_z)) {
                                    $current_zone_to = $current_z;
                                } else {
                                    /* Get Zone Id From Name */
                                    $carrierZonesNameFilter = new CarrierZonesFilter();
                                    $carrierZonesNameFilter->addIsDeletedFilter();
                                    //$carrierZonesNameFilter->addFieldLikeFilter('cz.name', $this->form_vars['zonename'][$current_z]);
                                    $carrierZonesNameFilter->addFieldFilter('TRIM(UPPER(cz.name))', trim(strtoupper($current_z)));
                                    ...
                                    if (count($zoneFilterObj) > 0) {
                                        $current_zone_to = $zoneFilterObj[0]->getId();

                                    }else if (!in_array($current_z, $missingZones)) {
                                        $missingZones[] = $current_z;
                                    }
                                    /* Get Zone Id From Name end */
                                }
                                $current_formula = $this->form_vars['formula'][$w];

                                /* Detail add query is here */
                                $from_zone_id = $this->form_vars['from_zone_id'];
                                $to_zone_id = $current_zone_to;
                                $weight_from = $w_from;
                                $weight_to = $w_to;
                                $user_weight_cost = !empty($z_price_weight_cost) ? $z_price_weight_cost : 0;
                                $user_peice_cost = !empty($z_price_piece_cost) ? $z_price_piece_cost : 0;
                    ...
                    foreach($tariffsCsv as $tariff) {
                        ...
                        }else if (!in_array($fromZoneName, $missingZones)) {
                            $missingZones[] = $fromZoneName;
                        }
                        /* Get Zone Id From Name end */
            ...
            $tariffsObj->logMoreDataNew['tariff_details'] = $tariff_details_data;
            $tariffsObj->logMoreDataOld['tariff_details'] = $tariff_details_data_old;
            $tariffsObj->tableKey = $tarif_id;
            $tariffsObj->saveAuditData();
            if (!empty($missingZones) && count($missingZones) > 0) {
                $missing_zones = implode(", ", $missingZones);
                $this->msg .= " Except these countries <b>".$missing_zones."</b>";
            }
		...
		if (isset($this->form_vars['func']) && $this->form_vars['func'] == 'get_service_agents') {
			$serviceId = $this->form_vars['service_id'];
			$allowedAgentId = "";
			$agentDataObj = AgentDataFilter::checkServiceAgent($this->serviceId);
			if (count($agentDataObj) > 0) {
				foreach ($agentDataObj as $agentData) {
					$allowedAgentId .= "'" . $agentData->getId() . "',";
				}
			}
			$allowedAgentId = rtrim($this->allowedAgentId, ",");
			$output = Ddl::generateDDL('agent_id', 'AgentDataFilter', "(agent_type = 'carrier' OR agent_type = 'both')", 'agent_name', 'id', '', ' class="form-control select2 agent_select"', "Please select agent", "", "agent", "Agent");
			echo $output;
			exit;
		}
...
//                    $output .= '<option value="">Select Zone To</option>';
        ...
        if (isset($this->form_vars['func']) && $this->form_vars['func'] == 'upload_csv_file') {
            ...
            $carrierId = $this->form_vars['carrier_id'];
            $tariffZoneType = $this->form_vars['type'];
            $templateType = $this->form_vars['template_type'];
                    ...
                    if (move_uploaded_file($csv_file['tmp_name'], $relPath)) {
                        ...
                        if (($handle = fopen($relPath, "r")) !== FALSE) {
                            while (($data = fgetcsv($handle)) !== FALSE) {
                                $num = count($data);
                                for ($c = 0; $c < $num; $c++) {
                                    if($templateType == 'kg' || $templateType == 'item') {
                                        if ($row == 1 && $c == 0) {
                                            $headings[] = $data[$c].'/'.$data[$c+1];
                                        }
                                        if ($row == 1 && $c > 1) {
                                            $headings[] = $data[$c];
                                        }
                                        if ($row > 1 && $c == 0) {
                                            $values[$row][] = $data[$c].'/'.$data[$c+1];
                                        }
                                        if($row > 1 && $c > 1){
                                            if($templateType == 'kg' && is_numeric($data[$c])) {
                                                $values[$row][] = $data[$c];
                                            }
                                            if($templateType == 'item' && is_numeric($data[$c])) {
                                                $values[$row][] = '0.00/'.$data[$c];
                                            }
                                            if(!is_numeric($data[$c])) {
                                                if($c < ($num - 1) && $templateType == 'kg') {
                                                    $values[$row][] = '0.00';
                                                } else if($c < ($num - 1) && $templateType == 'item') {
                                                    $values[$row][] = '0.00/0.00';
                                                } else {
                                                    $values[$row][] = $data[$c];
                                                }
                                            }
                                        ...
                                        if ($row == 1 && ($c == 0 || $c%2 == 0)) {
                                            $headings[] = $data[$c];
                                        } else if($row > 2 && $c%2 == 0 && $c < ($num - 1)) {
                                            $values[$row][] = $data[$c].'/'.$data[$c+1];
                                        } else if($row > 2 && $c == ($num - 1)) {
                                            $values[$row][] = $data[$c];
                                        }
                                    } else {
                                        if ($row == 1) {
                        ...
                        if($tariffZoneType == "single") {
                            ...
                            foreach ($headings as $key => $heading) {
                                if ($key == 0) {
                                    $html .= '<th class="bg-blue"><b>Weight/Zone</b></th>';
                                } else if ($key == (count($headings) - 1)) {
                                    $html .= '<th class="bg-blue"><b>' . $heading . '</b><input type="hidden" name="formulaname[]" value="' . $heading . '" ></th>';
                                } else {
                                    $html .= '<th class="bg-blue"><b>' . $heading . '</b><input type="hidden" name="zonename[]" value="' . $heading . '" ></th>';
                                }
                            }
                            $html .= '</tr>';
                            $html .= '</thead>';
                            ...
                            foreach ($values as $value) {
                                ...
                                foreach ($value as $k => $v) {
                                    $zone = trim($headings[$k]);
                                    if ($k == 0) {
                                        $weight = $v;
                                        $html .= '<td class="bg-info">';
                                        $html .= '<input type="text" name="weight[]" id="" class="form-control" value="' . $v . '">';
                                        $html .= '</td>';
                                    } else if ($k == (count($value) - 1)) {
                                        $html .= '<td>';
                                        $html .= '<input type="text" name="formula[' . $weight . ']" id="" class="form-control" value="' . $v . '">';
                                        $html .= '</td>';
                                    } else {
                                        $html .= '<td>';
                                        $html .= '<input type="text" name="zones[' . $weight . '][' . $zone . ']" id="" class="form-control" value="' . $v . '">';
                                        $html .= '</td>';
                                    }
                                }
                                $html .= '</tr>';
...
//                    $carrierZonesNameFilter->addFieldLikeFilter('cz.name', $zone);
        ...
        if (isset($this->form_vars['action']) && $this->form_vars['action'] == 'download_tariff_csv') {
            $tariffTemplate = $this->form_vars['tariff_template'];
			...
			$tariffsDetailsFilter->setLimit(100000);
            ...
            if (!empty($tariffDetailsCsvObj)) {
                ...
                foreach ($tariffDetailsCsvObj as $k => $tariffsDetailsObj) {
                    ...
                    if($tariffTemplate == "kg") {
                        $tariff_details[$weight][$zone_id] = $tariffsDetailsObj->getWeightCost();
                    } else if($tariffTemplate == "item") {
                        $tariff_details[$weight][$zone_id] = $tariffsDetailsObj->getPieceCost();
                    }  else {
            ...
            if($tariffZoneType == "multi") {
                ...
                if (!empty($tariffDetails)) {
                    foreach ($tariffDetails as $tariffsDetailsObj) {
                        ...
                        ksort($tariff_details[$zone_id]);
                    }
                    sort($zones);
                }
                $zones = $this->sort_zones($zones);
                $tariffDetailData = [];
                $endLimit = "";
                foreach($zones as $zone) {
                    ...
                    for ($weight = $minWeight; $weight <= $maxWeight;) {
                        if($this->numberFormat(($weight + $step)) <= $maxWeight){
                            ...
                            $tariffFound = 0;
                            foreach($zoneWeights as $weights => $arr){
                ...
                $tdhtml .= '<tbody>';
                $key = 0;
                foreach ($tariffDetailData as $weight => $zoneIds) {
                    $tdhtml .= '<tr>';
                    $tdhtml .= '<td class="bg-info">';
                    $tdhtml .= '<input type="text" name="weight[]" id="" class="form-control" value="' . $weight . '">';
                    $tdhtml .= '</td>';
                    foreach ($zoneIds as $zoneId => $dt) {
                        $tdhtml .= '<td>';
                        $tdhtml .= '<div class="row">';
                        $tdhtml .= '<div class="col-md-12">';
                        $tdhtml .= '<input type="text" name="zones[' . $weight . '][' . $zoneId . ']" id="" class="form-control" value="' . $dt['cost'] . '">';
                        $tdhtml .= '</div>';
                        $tdhtml .= '</div>';
                        $tdhtml .= '</td>';
                    }
                    $tdhtml .= '<td>';
                    $tdhtml .= '<div class="col-md-12">';
                    $tdhtml .= '<input type="text" name="formula[' . $weight . ']" id="" class="form-control" value="' . $dt['formula'] . '">';
                    $tdhtml .= '</div>';
                    $tdhtml .= '</td>';
                    $key++;
                    $tdhtml .= '</tr>';
        ...
        if (isset($this->form_vars['func']) && $this->form_vars['func'] == 'check_service_for_zone') {
            $serviceId = $this->form_vars['service_id'];
            $serviceObj = new Services($serviceId);
            $tariffType = $serviceObj->getTariffType();
            $output['type'] = $tariffType;
            echo json_encode($output);
            die;
        }
        ...
        if (isset($this->form_vars['action']) && $this->form_vars['action'] == 'download_tariff_template') {
            $serviceId = $this->form_vars['service_id_for_csv_download'];
            $templateTypeForCsvDownload = $this->form_vars['template_type_for_csv_download'];

            $serviceObj = new Services($serviceId);
            $tariffZoneType = $serviceObj->getTariffType();

            $zoneIds = $this->getValidZoneFromServiceId($serviceId);
            ...
            if($tariffZoneType == "single") {
                if (count($zoneIds)) {
                    if($templateTypeForCsvDownload == "default") {
                        $returnString = "";
                        $returnString .= "weight/zone,";
                        ...
                        $returnString .= "formula,";
                        $returnString .= "\r\n";
                    } else if($templateTypeForCsvDownload == "both") {
                        $returnString = "";
                        $returnString .= "weight/zone,";
                        foreach ($zoneIds as $zoneId) {
                            $carrierZone = new CarrierZones($zoneId);
                        ...
                        $returnString .= ",";
                        $returnString .= "\r\n";
                    } else if($templateTypeForCsvDownload == "kg" || $templateTypeForCsvDownload == "item") {
                        $returnString = "";
                        $returnString .= "weight from,";
                        $returnString .= "weight to,";
                        foreach ($zoneIds as $zoneId) {
                            $carrierZone = new CarrierZones($zoneId);
                            $returnString .= cleanCsvCall($carrierZone->getName()) . ",";
                        }
    ...
    protected function getValidZoneFromServiceId($serviceId) {
        ...
        $carrierZonesFilter->addFieldFilter('cz.carrier_id', $carrierId);
        if ($carrierZoneBase == 0) {
            $carrierZonesFilter->addFieldFilter('cz.service_id', $service->getId());
        }
        $carrierZones = $carrierZonesFilter->getColumnList('id,name');
    ...
    protected function sort_zones($zones) {
        $zoneFilter = new CarrierZonesFilter();
        $zoneFilter->addFilterIn('      cz.id',$zones);
        $zoneFilter->AddOrderBy('      cz.sort_order');
        $zoneFilter->AddOrderBy('      cz.name');
        $zoneFilterObjs = $zoneFilter->getList();
        if(count($zoneFilterObjs)) {
            $zones = [];
            foreach($zoneFilterObjs as $zoneFilterObj) {
                $zones[] = $zoneFilterObj->getId();
            }
        }
        return $zones;
    }
    ...
    public function addPagelavelJs()
    {
        ...
        <script type="text/javascript">
            function loadCarrierZones() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        ?>
                        $('#from_zone_id').val( <?php echo $this->tariffDetailsObj[0]->getFromZoneId(); ?> );
                        <?php
			...
			function loadServicesAgents(serviceId) {
				$.ajax({
					type: "POST",
					url: "tariff_add.php",
					data: {func: "get_service_agents", service_id: serviceId},
					dataType: "html",
					success: function (data) {
						$("#services_agent_span").html(data);
						$("#agent_id").select2();
					},
					error: function () {
						alert('error handing here');
            ...
            function check_service_for_zone() {
                var serviceId = $('#service_id').val();
                $.ajax({
                    type: "POST",
                    url: "tariff_add.php",
                    data: {func: "check_service_for_zone", service_id: serviceId},
                    dataType: "json",
                    success: function (res) {
                        $('#tariff_zone_type').val(res.type);
                        if(res.type == 'multi') {
                            $('.single_zone_tariff').hide();
                            $('.multi_zone_tariff').show();
                        } else {
                            $('.multi_zone_tariff').hide();
                            $('.single_zone_tariff').show();
                        }
            ...
            $(document).ready(function () {
                ...
                $("#upload_csv").click(function () {
                    ...
                    var tariffZoneType = $('#tariff_zone_type').val();
                    var templateType = $('#template_type').val();
                    var form_data = new FormData();
                    form_data.append('csv_file', file_data);
                    form_data.append('func', 'upload_csv_file');
                    form_data.append('type', tariffZoneType);
                    form_data.append('template_type', templateType);
                    $.ajax({
                        url: "tariff_add.php",
                        cache: false,
                        contentType: false,
            ...
            $(document).on('click', '#download_template_csv', function () {
                ...
                var templateType = $('#template_type').val();
                if(serviceId == "") {
                    swal("Sorry!", "Please select service", "error");
                } else {
                    $('#service_id_for_csv_download').val(serviceId);
                    $('#template_type_for_csv_download').val(templateType);
                    $('#template_download_form').submit();
                }
            });
    ...
    protected function renderBody()
    {
        ...
        <div class="portlet light">
            <div class="portlet-title">
                ...
                <div class="actions">
                    <?php if($this->tarifObj->getTariffType() == "customer") { ?>
                    <?php
                        $startDate = formatDate(date("d-m-Y", strtotime($this->tarifObj->getStartDate())));
                        $endDate = formatDate(date("d-m-Y", strtotime($this->tarifObj->getEndDate())));
                    ?>
                    <a href="javascript:;" class="btn btn-sm blue" id="download_tariff_excel" data-id="<?php echo $this->tarifObj->getId() ?>" data-start_date="<?php echo $startDate ?>" data-end_date="<?php echo $endDate ?>" data-description="<?php echo $this->tarifObj->getDescription() ?>" >
                        <span></span><i class="fa fa-download"></i>&nbsp;
                        Download Customer Tariff
                    </a>
                    <?php } ?>
                    <?php
                    $slectedCarrierId = '';
                    ...
                    <?php if(isset($_GET['id']) && !empty($_GET['id'])) {
                        ...
                        <a href="javascript:;" class="btn btn-sm blue" data-toggle="modal" id="span_user_service_remotearea_<?php echo $_GET['id']; ?>" data-target="#model_remoterea_supplier" onclick="return <?php echo $funcRemoteArea ?>" title="Tariff Remoteareas Charges"><span></span><i class="fa fa-money"></i>&nbsp; Tariff Remoteareas Charges </a>
                        <a href="tariff_additional_charges.php?tariff_id=<?php echo $_GET['id']; ?>" class="btn btn-sm blue"><span></span><i class="fa fa-money"></i>&nbsp; Tariff Additional Charges </a>
            ...
            <div class="portlet-body">
                ...
                <form name="tarif_form" id="tarif_form" action="" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="tariff_zone_type" id="tariff_zone_type" value="single" >
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group input-group-sm">
                                    <span class="input-group-addon"> <i class="fa fa-shopping-cart"></i></span>
                                    <?php
                                    echo Ddl::generateDDL('currency_id', 'CurrencyFilter', "AND isactive='1' AND ClientDisplay='1'", 'currencyname', 'id', $this->tarifObj->getCurrencyId(), ' class="input-sm form-control select2 validate_check"', 'Pound Sterling', '2');
                                    ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-3 single_zone_tariff">
                            <div class="form-group">
                                <label>From Zone</label>
                                <div class="input-group input-group-sm">
                                    <span class="input-group-addon"> <i class="fa fa-shopping-cart"></i></span>
                                    <span id="zone_from_span">
                                        <select name="from_zone_id" id="from_zone_id" class="form-control select2 validate_check from_zone_id">
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
								...
								<select class="form-control select2" name="chkType" id="chkType">
									<option value="customer"<?php echo($chkType == 'customer' ? ' selected="selected"' : ''); ?>>Customer</option>
									<option value="supplier"<?php echo($chkType == 'supplier' ? ' selected="selected"' : ''); ?>>Supplier</option>
									<option value="agent"<?php echo($chkType == 'agent' ? ' selected="selected"' : ''); ?>>Agent</option>
								</select>
						...
						<div class="col-sm-3" id="agent_box">
							<div class="form-group">
								<label>Tariff Agent</label><br/>
								<span id="services_agent_span">
								<?php
									$selected_value = $this->tarifObj->getAgentId();
									$_serviceId = $this->tarifObj->getServiceId();
//									if($_serviceId > 0) {
//										$agentDataObj = AgentDataFilter::checkServiceAgent($this->serviceId);
//										if (count($agentDataObj) > 0) {
//											foreach ($agentDataObj as $agentData) {
...
//											}
//										}
										echo Ddl::generateDDL("agent_id", 'AgentDataFilter', "(agent_type = 'carrier')", 'agent_name', 'id', $selected_value, ' class="form-control select2 agent_select"', "Please select agent", "", "agent", "Agent");
//									} else {
									?>
...
//									}
                        ...
                        <div class="col-sm-2">
                         ...
                         <div class="col-sm-4 tariff_csv_box_1">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group input-large">
                                        ...
                                        <span class="input-group-addon btn default btn-file">
                                            ...
                                            <input type="file" name="tariff_file" id="tariff_file">
                                        </span>
                                        <a href="javascript:;" class="input-group-addon btn red fileinput-exists"
                                           data-dismiss="fileinput"> Remove </a>
<!--                                     <a href="javascript:;" class="input-group-addon btn blue"
                                           id="upload_csv2">Upload</a>
-->
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <div class="form-group">
                                ...
                                <?php $templateType = $this->tarifObj->getTariffTemplate(); ?>
                                <select name="template_type" id="template_type" class="form-control select2">
                                    <option value="default" <?php echo($templateType == 'default' ? 'selected="selected"' : ''); ?> >Default</option>
                                    <option value="kg" <?php echo($templateType == 'kg' ? 'selected="selected"' : ''); ?> >Kg Cost Only</option>
                                    <option value="item" <?php echo($templateType == 'item' ? 'selected="selected"' : ''); ?> >Item Cost Only</option>
                                    <option value="both" <?php echo($templateType == 'both' ? 'selected="selected"' : ''); ?> >Both Kg & Item Cost</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-6 tariff_csv_box">
                            <div class="fileinput fileinput-new" data-provides="fileinput">
                                <div class="form-group">
                                    <label> Upload Your Document</label>
                                    <div class="input-group input-large">
                        ...
                        <div class="col-sm-2 text-right">
                            <label>&nbsp;</label><br />
                            <a href="javascript:;" class="btn blue tariff_csv_box" id="download_template_csv" ><i class="fa fa-download"></i> Template</a>
                        </div>
                    </div>
                            ...
                            <div class="col-md-12">
                                <div class="table-responsive">
                                    <table class="table">
                                        <thead>
                                        <tr>
                                            <th width="200" class="multi_zone_tariff"><label>From Zone</label></th>
                                            <th width="200"><label>To Zone</label></th>
                                            <th><label>Weight</label></th>
                                            <th class="item_cost"><label>Item Cost</label></th>
                                            <th class="kg_cost"><label>KG Cost</label></th>
                                                        ...
                                                        <select name="tariffs[<?php echo $k + 1; ?>][formula]" class="form-control select2 formula input-sm">
															...
															if(!empty($tariffFormulas)){
																foreach($tariffFormulas as $tariffFormula => $tariffFormulaText){
																	$selected = "";
																	if($tariffsDetailsObj->getFormula() == $tariffFormula){
                ...
                <form id="csv_download_form" name="csv_download_form" method="post">
                    <input type="hidden" name="tariff_id" id="tarif_id_for_csv_download" value="<?php echo $this->tariffId; ?>"/>
                    <input type="hidden" name="tariff_template" id="tariff_template_csv_download" value="<?php echo $this->tarifObj->getTariffTemplate(); ?>"/>
                    <input type="hidden" name="action" value="download_tariff_csv"/>
                </form>
                <form id="template_download_form" name="template_download_form" method="post">
                    <input type="hidden" name="service_id_for_csv_download" id="service_id_for_csv_download" value=""/>
                    <input type="hidden" name="template_type_for_csv_download" id="template_type_for_csv_download" value=""/>
                    <input type="hidden" name="action" value="download_tariff_template"/>
                </form>
        ...
        <form id="download_tariff_excel_form" action="tariffs_list.php" method="POST">
    ...
    protected function renderHead() {
        ...
        <style>
            ...
            .tariffs-container .select2-container {
