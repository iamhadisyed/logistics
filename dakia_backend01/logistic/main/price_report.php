...
class Page extends BasePage {
    ...
    protected function removeChar($text) {
        $textNew = $text;
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'download') {
            ...
            if (trim($batch_number) != '') {
                ...
                if (count($res) > 0) {
                    ...
                    $returnString = "Invoice Type, Invoice Number, Supplied Account, Customer Account,  Tracking Number, Supplier Code, Service Code, Country, Customer Order Number,Dispatched Date, Weight, Sys Vol Wt, Cal Vol Wt., Charged Price, Price to be charges, Price Margin, Message \r\n";
                    foreach ($res as $invoiceData) {
                        $returnString .= $this->removeChar($invoiceData->getInvoiceType()) . ",";
                        $returnString .= $this->removeChar($invoiceData->getInvoiced()) . ",";
                        ...
                        $returnString .= $this->removeChar($invoiceData->getCountry()) . ",";
                        ...
                        $returnString .= $this->removeChar($invoiceData->getVolWeight()) . ", ";
                        $returnString .= $this->removeChar($invoiceData->getCalWeight()) . ", ";
                        ...
                        $returnString .= $this->removeChar($invoiceData->getBatchNumber()) . ", ";
                        $returnString .= "\r\n";
    ...
    protected function renderHead() {
        ...
        <script type="text/javascript">
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status == 'success') {
                                $('#console_window').append('Start calculating data<br />');
            ...
            function importCSV(startfrom, batch_number) {
                ...
                $('#console_window').append('Pricing calculation records from ' + (startfrom + 1) + ' to ' + (startfrom + batch_limit) + '...');
                var form_data = new FormData();
                form_data.append('action', 'GET_PRICING_DETAILS');
                form_data.append('request', 'ajax');
                form_data.append('start', startfrom);
                form_data.append('batch_number', batch_number);
                $.ajax({
                    url: 'pricing_data_m.php',
                    dataType: 'json',
                    cache: false,
                    contentType: false,
                    processData: false,
                    ...
                    error: function (response) {
                        $('#console_window').append(JSON.stringify(response));
                        // importCSV(startfrom,batch_number)
                    }
                });
            }
        </script>
