...
class Page extends BasePage {
    ...
    protected function init() {
        if (!Permissions::checkFilePermission('carrier_list.php'))
...
//          Save Service remotearea
        ...
        if (isset($this->form_vars["get_all"]) && $this->form_vars["get_all"] == "carrier_list") {
            ...
            foreach ($carrierResult as $rowImage) {
                ...
                if ($zoneBase == 1) {
                    $carrierBaseHtml = (Permissions::checkFilePermission('carrier_tariff_list') ? '<td><a class="dropdown-item" href="tariffs_list.php?carrier_id=' . $rowImage->getId() . '" data-carrier_id = "' . $rowImage->getId() . '" >Tariff List</a></td>' : '')
                        . (Permissions::checkFilePermission('carrier_add_zone') ? '<td><a class="dropdown-item" href="carrier_zones.php?carrier_id=' . $rowImage->getId() . '" data-carrier_id = "' . $rowImage->getId() . '" > </a></td>' : '');
                }
                $html .= '<li>
                                    
                        ' . (trim($status) == '1' ? '<a class="dropdown-item carrier_status status_' . $carrierId . '" data-status="inactive" data-carrier_id="' . $carrierId . '"  data-carrier_name="' . $displayName . '">  ACTIVE </a></li>' :
                        '<a class="dropdown-item carrier_status status_' . $carrierId . '"  data-status="active"  data-carrier_id="' . $carrierId . '" data-carrier_name="' . $displayName . '">  INACTIVE </a>') . '
                                
                        ' . (Permissions::checkFilePermission('carrier_edit') ? '<li><a  class="dropdown-item carrier_edit" data-carrier_id = "' . $rowImage->getId() . '" >
                              EDIT
                          </a></li>' : '') ;
                $html .= (Permissions::checkFilePermission('carrier_delete') ? '<li><a class="dropdown-item delete_carrier " href="#" data-carrierId = "' . $rowImage->getId() . '">Delete</a></li>' : '');

                $html .= (Permissions::checkFilePermission('carrier_audit') ?  "<li><a href='' class='dropdown-item' id='user-audit-detail-view' data-target='#user-audit-view-modal' data-log_key='" . $rowImage->getId() . "' data-log_name='carrier' data-toggle='modal'>Audit</a></li>": '');

                $html .= (Permissions::checkFilePermission('carrier_show_services') ? '<li><a class="dropdown-item event-carrierload" 
                ...
                $html .= (Permissions::checkFilePermission('carrier_transit_time') ? '<li><a class="dropdown-item" href="service_country_time.php?carrier_id=' . $rowImage->getId() . '" data-carrier_id = "' . $rowImage->getId() . '" rel="tooltip" title="Transit Time">
                                                 Carrier Transit Time
                                            </a></li>' : '');


                $html .= (Permissions::checkFilePermission('carrier_document_upload') ? ''
                        . '<li> <a '
                        . 'class="dropdown-item carrier_document_upload" '
                        . ' '
                        . 'data-carrier_name="' . $rowImage->getCarrier() . '" '
                        //. 'data-toggle="modal" '
                ...
                $html .= '<td><img class="img-responsive center-block" src="' . $logoPath . '" alt="' . $rowImage->getCarrier() . '" uib-popover="' . $rowImage->getCarrier() . '" popover-trigger="mouseenter" > </td>';
                $html .= '<td>' . $cutOffTime . ' </td>';
                $html .= '<td>' . $currencyCode . ' </td>';
                $html .= '<td>' . $onContract . ' </td>';
            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header align-items-center d-flex">
                        <h4 class="card-title mb-0 flex-grow-1">Carrier List</h4>

                        <div class="flex-shrink-0">
                            <a href="carrier.php" class="btn btn-primary">Add New Carrier</a>
                            <?php
                            if (Permissions::checkFilePermission('carrier_view_services')) {
                                ?>
                                <a href="services_list.php" class="btn btn-primary">List Services</a>
                                <?php
                            }
                            ?>
                        ...
                        <div class="live-preview">
                            <div class="table-responsive table-card">
                                <table class="table align-middle table-nowrap table-striped-columns mb-0" >
                                    ...
                                    <tbody id="append_all_carrier_list">
                                    <tr>
                                        <td>
                                            <div class="dropdown">
                                                <a href="#" role="button" id="dropdownMenuLink1" data-bs-toggle="dropdown" aria-expanded="false">
...
<!--                            <input type="text" class="form-control input-circle" placeholder="search..." name="search_carrier" id="search_carrier"> </div>-->
<!--                    </div>-->
<!--                </div>-->
<!--            </div>-->
<!---->
