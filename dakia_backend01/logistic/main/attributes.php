...
class Page extends BasePage
{
    ...
    protected function init()
    {
        ...
        if (isset($this->form_vars["action"]) && $this->form_vars["action"] == "save") {
            ...
            if ($name == "") {
                $output['status'] = "error";
                $output['message'] = "Please enter arrtibute";
                $validate = false;
            }
            ...
            if ($validate) {
                ...
                $attributes->setAttrbuiteName($name);
                $attributes->setAttrbuiteGroup($group);
                $attributes->setIsActive($isActive);
                $attributes->setDateAdded($dateAdded);
                $attributes->setAddedBy($addedBy);
                $attributes->setDateUpdated($dateUpdate);
                $attributes->setUpdatedBy($updateBy);
                $attributes->save();
                $output['status'] = "success";
                $output['message'] = "Attributes saved successfully";
            }
            echo json_encode($output);
            die;
        }

        if (isset($_GET['action']) && $_GET['action'] == "attributes_list") {
            ...
            if (isset($this->form_vars['action']) && $this->form_vars['action'] == 'filter') {
                $name = $this->form_vars['name'];
                if (!empty($name)) {
                    $attributesFilter->whereLike(['name' => $name]);
                }
                $group = $this->form_vars['group'];
                if (!empty($group)) {
                    $attributesFilter->whereLike(['group' => $group]);
                }
                $isActive = $this->form_vars['is_active'];
                if (!empty($isActive) || $isActive == 0) {
                    $attributesFilter->where(['is_active' => $isActive]);
                }
                ...
                if (!empty($searchDateFrom) && !empty($searchDateTo)) {
                    $attributesFilter->whereBetween('date_created', $searchDateFrom, $searchDateTo);
                }
            }
            /*
            ...
            if (isset($this->form_vars['order'][0]['column']) && $this->form_vars['order'][0]['column'] != 0) {
                ...
                if ($dataTableColumnName != "action") {
                    $attributesFilter->orderBy(strtolower($dataTableColumnName), $orderFalse);
                }
            } else {
                $attributesFilter->orderBy(strtolower("a.id"), "DESC");
            }
            /*
            ...
            $attributesFilter->setRowsPerPage($iDisplayLength);
            $attributesFilter->setOffset($iDisplayStart);
            $attributesFilterObjs = $attributesFilter->getList("a.*");
            $iTotalRecords = $attributesFilter->getCount();
            $setDataArr = array();
            foreach ($attributesFilterObjs as $attributesFilterObj) {
                $currentArr['name'] = $attributesFilterObj->getAttrbuiteName();
                $currentArr['group'] = $attributesFilterObj->getAttrbuiteGroup();
                $isActive = '<div class="text-center"><span class="label label-sm label-danger">No</span></div>';
                if ($attributesFilterObj->getIsActive() == 1) {
                ...
                $action = '<div class="btn-group" data-container="body" >
                    ...
                    $action .= '<li>
                                    <a title="Edit" href="javascript:;" data-id="' . $attributesFilterObj->getId() . '" class="edit">
                                        <span class="glyphicon glyphicon-eye-open"></span> Edit
                                    </a>
                                </li>';
                    $action .= '<li>
                                    <a title="Delete" href="javascript:;" data-id="' . $attributesFilterObj->getId() . '" class="delete">
        ...
        if (isset($this->form_vars["action"]) && $this->form_vars["action"] == "edit") {
            $id = $this->form_vars['id'];
            $attributeObj = new Attributes($id);
            $dt = [
                'id' => $attributeObj->getId(),
                'name' => $attributeObj->getAttrbuiteName(),
                'group' => $attributeObj->getAttrbuiteGroup(),
                'is_active' => $attributeObj->getIsActive()
            ];
            $return = [
                'status' => 'success',
                'attribute' => $dt
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function (e) {
                $(document).on('click', '#btnSave', function () {
                    var form = $('#attributes_form')[0]; // You need to use standard javascript object here
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.status == "success") {
                                ...
                                var group = data.attribute.group;
                                var is_active = data.attribute.is_active;
                                $('#id').val(id);
                                $('#name').val(name);
                                $('#group').val(group);
                                if (is_active == 1) {
    ...
    protected function renderBody()
    {
        ...
        <div class="portlet light">
            ...
            <div class="portlet-body">
                <form action="" method="post" enctype="multipart/form-data" id="attributes_form" name="attributes_form">
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <label class="label-account">Attributes Name</label>
                            <input type="text" class="form-control" name="name" id="name">
                        </div>
                        <div class="col-md-3">
                            <label class="label-account">Attributes Group</label>
                ...
                <div class="table-container">
                    ...
                    <table class="table table-striped table-bordered table-hover table-condensed" id="manage-data-table">
                        ...
                        <tr role="row" class="filter">
                            ...
                            <td>
                                <input type="text" class="form-control form-filter" name="group"/>
                            </td>
                            <td>
                                <?php
