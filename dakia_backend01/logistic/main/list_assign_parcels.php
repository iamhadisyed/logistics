...
class Page extends BasePage
{
    ...
    protected function init()
    {
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'delete_parcel_consignment') {
            ...
            if ($parcel_assignment_id == 'all') {
                $assignVehicleFilter->unassign_parcels(0, true);
            } elseif (!empty($parcel_assignment_id)) {
                $assignVehicleFilter->unassign_parcels($parcel_assignment_id);
            }
            $setDataArrJson['status'] = 'success';
            $setDataArrJson['message'] = 'Record Updated Successfully.';
        ...
        if (isset($_GET['action']) && $_GET['action'] == "parcels_list_ajax") {
            ...
            if (isset($this->form_vars['action']) && $this->form_vars['action'] == 'filter') {
                ...
                $pickup_date = !empty($this->form_vars['pickup_date']) ? date('Y-m-d', strtotime($this->form_vars['pickup_date'])) : '';
...
//                $currentArr['vehicle_type'] = $vehiclesListObj->getVehicleType();
                ...
                $currentArr['pickup_date'] = formatDate($vehiclesListObj->getPickupDate());
                $currentArr['hawb'] = $vehiclesListObj->getHawb();
                ...
                $currentArr['service_name'] = $vehiclesListObj->getServiceName();
                $currentArr['parcel_status_code'] = Consignment::getShipnmentStatus($vehiclesListObj->getShipmentStatus());
                $currentArr['address'] = $vehiclesListObj->getAddressLine1() . ' ' . $vehiclesListObj->getAddressLine2() . ' ' . $vehiclesListObj->getAddressLine3() . ' ' . $vehiclesListObj->getCity() . ' ' . $vehiclesListObj->getState() . ' ' . $vehiclesListObj->getPostCode() . ' ' . $vehiclesListObj->getCountryName();
//                $currentArr['driver_data'] = $driverData;
                if ($vehiclesListObj->getShipmentStatus() == 19) {
                    $currentArr['actions'] = '';
                } else {
                    $currentArr['actions'] = '<div>';
                    $currentArr['actions'] .= '<span class="action_check_box">';
                    $currentArr['actions'] .= '<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"><input type="checkbox" name="multi_select[]"  data-parcel_id="' . $vehiclesListObj->getParcelId() . '" id="multi_select_' . $vehiclesListObj->getParcelId() . '"  value="' . $vehiclesListObj->getParcelId() . '"  class="group-checkable parcelIds shipment-check" /><span></span></label>';
                    $currentArr['actions'] .= '</span>';
                    $currentArr['actions'] .= "<a href='javascript:void(0);' data-parcel_assignment_id='" . $vehiclesListObj->getId() . "' class='btn btn-danger btn-xs delete_vehicle_assignment'><i class='fa fa-trash'></i></a>";
                    $currentArr['actions'] .= '</div>';
                }
    ...
    public function addPagelavelJs()
    {
        ...
        <script type="text/javascript">
                ...
                var handleDataTable = function () {
                    ...
                    grid.init({
                        ...
                        dataTable: {// here you can define a typical datatable settings from http://datatables.net/usage/options
                            ...
                            "ajax": {
                                "url": "list_assign_parcels.php?action=parcels_list_ajax", // ajax source
                                headers: {},
                            },
                            "bStateSave": true,
                            "columns": [
            ...
            $(document).ready(function () {
                ...
                $(document).on("click", "#unassign_vehicle_all_btn", function () {
                    var parcel_assignment_id = 'all';
                    swal({
                            title: "Are You Sure?",
                            text: "you want to unassign all parcels!",
                            type: "warning",
                        ...
                        function (isConfirm) {
                            if (isConfirm) {
                                $.ajax({
                                    url: 'list_assign_parcels.php?action=delete_parcel_consignment',
                                    data: {
                ...
                $(document).on("click", "#unassign_vehicle_selected_btn", function () {
                    var ids = [];
                    $('.parcelIds:checked').map(function () {
                        ids.push(this.value);
                    }).get();
                    if (typeof ids !== 'undefined' && ids.length > 0) {
                        var parcel_assignment_id = ids;
                        swal({
                                title: "Are You Sure?",
                                text: "you want to unassign selected parcels!",
                                type: "warning",
                                showCancelButton: true,
                                confirmButtonClass: "btn-danger",
                                confirmButtonText: "Yes",
...
//        }
        ...
        <div class="portlet light">
            <div class="portlet-title">
                ...
                <div class="actions" id="bluk_actions">
                <a href="javascript:;" class="btn btn-default" id="unassign_vehicle_all_btn">
                    Unassign all Parcels
                </a>
                <a href="javascript:;" class="btn btn-default" id="unassign_vehicle_selected_btn">
        ...
        <div class="portlet-body">
            ...
            <table class="table table-striped table-bordered table-hover table-condensed" id="manage-data-table">
                ...
                <tr role="row" class="filter">
                    ...
                    <td class="user_acccount_correct_button">
                        <input type="text" class="form-control form-filter" name="hawb"
                               id="hawb"/>
                    </td>
                    <td class="user_acccount_correct_button">
                    ...
                    <td class="user_acccount_correct_button">
                        <input type="text" class="form-control form-filter" name="service_name"
                               id="service_name"/>
                    </td>
                    <td class="user_acccount_correct_button">
                        <input type="text" class="form-control form-filter" name="parcel_status_code"
                               id="parcel_status_code"/>
                    </td>
                    <td class="user_acccount_correct_button">
                        <input type="text" class="form-control form-filter" name="address"
                               id="address"/>
                        <?php echo Ddl::generateCountryDDL('search_Country', '', 'id'); ?>
                    </td>
                </tr>
                </thead>
                <tbody>
