...
foreach ($orderReferenceArray as $hawb)
{
    ...
    if(count($result > 0))
    {
        $tableId = $result[0]->getId(); // MarketPlace table id 
        $marketPlaceObj = new MarketPlaceOrder($tableId);
    }
    ...
    $resultValue = invokeSubmitFeed($service, $request, $hawb, $marketPlaceObj, $consignmentId);
...
function invokeSubmitFeed(MarketplaceWebService_Interface $service, $request, $hawb, $marketPlaceObj, $consignmentId) 
  {
    try {
