...
<!--        <script src="../assets/global/plugins/counterup/jquery.counterup.min.js" type="text/javascript"></script>-->
        ...
        <script type="text/javascript">
            ...
            var getTotalShipments = function () {
                ...
                var user_type = "<?php echo preg_match('/^[a-zA-Z]+$/', $_SESSION['menu-option']) ? $_SESSION['menu-option'] : ''; ?>";
                $.ajax({
        ...
        if($this->userSession->getDashboard() == "operation" && $this->userSession->getUserType() != "client"){
            ...
            include_once 'dashboard/customer_serivces_js.php';
    ...
    public function init() {
        ...
        if (!isset($_POST['func']) && $userSection->getDefaultLang() != 'en-GB' && $userSection->getDefaultLang() != '') {
            ...
            if (trim($lang_session) != '' && $lang_selected != '' && $lang_session != $lang_selected) {
                ...
                util_redirect(BASE_URL . "dashboard?lang=" . trim($lang_selected));
            } else if (trim($lang_session) != '') {
                
            } else if (trim($lang_default) != '') {
                $_SESSION['lang'] = $lang_default;
                util_redirect(BASE_URL . "dashboard?lang=" . $lang_default);
            }
        }
        ...
        if (!isset($_SESSION['admin']['id']) OR is_null($_SESSION['admin']['id'])) {
            ...
            if (trim($data) && $data != '') {
                ...
                util_redirect(BASE_URL . "login?data=" . $data);
            } else {
                //util_redirect(SETTING_MAIN_URL . "main/login.php");
                util_redirect(BASE_URL . "login");
            }
        }
        ...
        if (isset($_REQUEST['func']) && $_REQUEST['func'] == 'GET_TOTAL_SHIPMENTS') {
            ...
            if ($user_type == User::USER_TYPE_CLIENT)
                $cFilter->addFilter("     c.user_id = '" . $userSection->getId() . "'");
            else if ($user_type == User::USER_TYPE_CORPORATE) {
                $cFilter->addFilter("     ( c.user_id = '" . $userSection->getId() . "'  OR c.user_id IN (SELECT id FROM user WHERE user_account_id = '" . $userSection->getUserAccountId() . "' ))");
            }
