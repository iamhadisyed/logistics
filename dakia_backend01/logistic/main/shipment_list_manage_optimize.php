    ...
    protected function init() {
        ...
        if (isset($_GET['action']) && $_GET['action'] == "consignment_list") {
            ...
            if (isset($this->form_vars['action']) && $this->form_vars['action'] == 'filter') {
                ...
                $getZeroPricedObj = $this->consignment_filter->getZeroPricedCount();
                $getZeroPriced = 0;
                ...
                foreach ($consignmentObjs as $consignmentObj) {
                    ...
                    $consignmentChargesFilterObj = $consignmentChargesFilter->getColumnList("sum(cc.cost) as cost, cost_currency, cc.charge_type_id,cc.cost_type");
                    $totalTariffCost = 0;
                    ...
                    $consignmentArr['account_mawb_dateDispatched'] = $returnIcon.'<a href="javascript:;" onclick="get_user_detail(' . $consignmentObj->getUserId() . ')" >' . $consignmentObj->getUserAccount() . '</a><br />' . $consignmentObj->getMawb() . '<br />' . $dateBooked;
                    $consignmentArr['status_hawbNumber_trackingNumber'] = $shipment_status . '<br />' . $hawb . '<br />' . $trackingNumber;
                    $consignmentArr['location'] = $shipmentType . '<br />' .utf8_encode(  $consignmentObj->getCity() ). '<br />' . utf8_encode(  $countryObj->getName());
        ...
        if (isset($_GET['action']) && $_GET['action'] == "update_manaual_pod") {
            ...
            $barcodelist = trim($this->form_vars['trackingNumber']);
            ...
            if (count($this->form_vars['multi_select']) > 0) {
                ...
                if (count($bulk_trackingNumber) > 0) {
                    ...
                    foreach ($bulk_trackingNumber as $trackingNumer) {
                        $bulkTrackingNumberValidatedArray[] = $trackingNumer;
                    }
                }
            ...
            if (sizeof($bulkTrackingNumberValidatedArray) > 0) {
                ...
                if (isset($_FILES['pod_file']) && !empty($_FILES['pod_file']['name'])) {
                    ...
                    $imageName = "bulk_manual_pod_" . $_FILES['pod_file']['name'];
                    if (!file_exists("../_assets/images/pod_images")) {
                        mkdir("../_assets/images/pod_images", 0777, true);
                    }
                    $targetPath = "../_assets/images/pod_images/" . $imageName;
                ...
                TrackingData::AddVirtualTrackingToScanParcels($bulkTrackingNumberValidatedArray, $sessionUser, $pod_value, $date_delivery, $emailContentForPods, $track_point, $imageName, '', '', $pod_name);
...
//                    $invoiceDetailData->save();
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'update_vol_weight') {
            ...
            if (isset($_FILES['vol_weight_file']) && !empty($_FILES['vol_weight_file']['name'])) {
                ...
                $targetPath = "../_assets/vol_weight/" . $imageName;
                if (move_uploaded_file($sourcePath, $targetPath)) {
                    $row = 1;
        ...
        if (isset($_GET['action']) && $_GET['action'] == 'update_shipnment_status') {
            $output = [];
        ...
        if (isset($this->form_vars['csv_action']) && $this->form_vars['csv_action'] == 'download_shipnments') {
            ...
            $heading = ["Account", "Invoice No", "Order Ref No", "Tracking Number", "MAWB", "Bag No", "Service Code", "Service Name", "Service Type", "Product Name", "REF", "Consignment Status", "Date Label Created", "Date Scanned", "Date Dispatched", "Date Delivered", "Collection Country", "Collection  Postcode", "Company", "Address", "City", "Country", "Postcode", "Number of Pieces", "Chargable Weight", "Weight", "Vol Weight", "DOX/NDX", "Value", "Currency", "Remotearea", "Length", " Width", "Height"];
        ...
        if (isset($this->form_vars['csv_action']) && $this->form_vars['csv_action'] == 'download_csv') {
          ...
          $columns = " c.user_id,c.awb,c.mawb,c.service_id,c.shipment_status,c.date_scanned,c.date_label_created, "
                    . "  c.company,c.contact,c.address_line_1,c.address_line_2,c.address_line_3, "
                    . "  c.city,c.country_id,c.postcode,c.telephone, c.description,c.number_pieces,c.weight, "
                    . "  c.charge_weight,c.vol_weight,c.value,c.currency,c.remote_charges,c.hawb,c.reference, "
                    . " c.date_booked,c.is_doc,c.consignment_status,ua.user_account, c.is_dead_weight_chargable, "
                    . " c.routing_code_eur, c.consignment_type,cc.invoice_id AS charges_invoice_id ,inv.invoice_no as invoice_no , cbm.bagid "
                    . ", pc.length,pc.height,pc.width , pc.owe_status_code , pc.last_tracking_update ,    
                        (SELECT date_created FROM tracking_data td WHERE status_code_id IN('122','121')  AND pc.tracking_number = td.tracking_number order by date_created limit 1)  as date_delivered 
                    ";
            $this->consignment_filter->addInvoiceJoin();
            $this->consignment_filter->addBaggingMappingJoin();
            $consignmentObjs = $this->consignment_filter->getColumnListOptimized($columns);
            ...
            if ($includeAgent == 1) {
                ...
                $consignmentChargeTypeAgentObj = $consignmentChargeAgentType->getList("id");
                 if(count($consignmentChargeTypeAgentObj)>0){
                    foreach($consignmentChargeTypeAgentObj as $consignmentAgent){
                        $this->agentChargesTypeArray[$consignmentAgent->getId()] = $consignmentAgent->getTitle(); 
                    }
                }
            }
            ...
            if (!empty($consignmentObjs)) { 
                        ...
                        $output = $this->downloadCSV($consignmentObjs, $includeAgent, $consignmentChargeTypeCustomerObj, $consignmentChargeTypeAgentObj, $action_ext);
                        }
                ...
                if (count($invoiceIds) > 0) {
                    foreach ($invoiceIds as $invoiceId) {
                        ...
                        if (count($consignmentFilterObj) > 0) {
                            ...
                            $returnData = $sammaryPdf->SaveSummaryInvoicePDFFile($chargesData, $invoiceId, $checks);
                        ...
                        $pdf = new InvoicePDF();
                        $data = $pdf->SavePDFFile($consignments, $invoiceId, $checks);
                        $invoiceObj = new Invoices($invoiceId);
        ...
        if (isset($this->form_vars['action']) && $this->form_vars['action'] == "gp_report_excel") {
            ...
            if ($this->form_vars['report_type'] == 'detail') {
                             ...
                             $dataColumns = "c.awb,c.mawb,c.date_label_created, c.number_pieces,c.weight,c.charge_weight,c.vol_weight, c.hawb,c.reference,ua.user_account, c.is_dead_weight_chargable,s.name as service_name,c.id ,"
                ...
                if (!empty($consignmentObjs)) {
                    $output = $gpReportObj->downloadExcelDetailGpReport($consignmentObjs,  $loggedInUserCurrencySymbol);
                }
                   ...
                   . "  
                       ...
                       ";
                        $consignmentObjs = $this->consignment_filter->getColumnList($dataColumns, 25000  ); 
                          ...
                          if (!empty($consignmentObjs)) {
                                $output = $varianceReportObj->downloadExcelVarianceReport($consignmentObjs,  $loggedInUserCurrencySymbol);
                            }
    ...
    protected function trackScriptExcecution($steps){
        $time_end = microtime(true);
        //dividing with 60 will give the execution time in minutes otherwise seconds
        $execution_time = ($time_end - $this->startTime) ;
        //execution time of the script
        echo "<br><b>".$steps.": </b> ".round(($execution_time/60),3).' Mins '.round($execution_time,3).' Seconds <br>';
    }
...
//            echo "<pre>";
       ...
       $this->consignment_filter->addConsignmentChargesJoin($this->user->getUserAccountId(), $this->user->getUserType() );
        ...
        if (count($consignmentIdsSearchInFilter) > 0) {
            ...
            if((trim($mawb) != '' && $mawbFilterCheck) || (trim($manifest) != '' && $manifestFilterCheck)) {
    ...
    function downloadCSV($dataConsignment, $agentOption = 0, $consignmentChargeTypeCustomerObj = array(), $consignmentChargeTypeAgentObj = array(), $action_ext) {
        ...
        if (count($dataConsignment) > 0) {
            ...
            if ($currentConsignmentAdd == 0) {
                fwrite($myfile, $this->exportHeader($agentOption, $consignmentChargeTypeCustomerObj, $consignmentChargeTypeAgentObj, $action_ext));
            }
            ...
            foreach ($dataConsignment as $consignmentItem) {
                ...
                $rowTowrite = $this->exportRowCsv($consignmentItem, $agentOption, $customerChargesIds, $agentChargesIds, $action_ext );
    ...
    function exportRowCsv($consignmentObj, $agentOption = 0, $customerChargesIds, $agentChargesIds, $action_ext) {
        ...
        $returnString .= cleanCsvCall($consignmentObj->getInvoiceNo()) . ",";
        ...
        $returnString .= cleanCsvCall($consignmentObj->getBagid()) . ",";
        $returnString .= cleanCsvCall($this->servicesDataArray[$consignmentObj->getServiceId()]['code']) . ",";
        $returnString .= cleanCsvCall($this->servicesDataArray[$consignmentObj->getServiceId()]['name']) . ",";
        $returnString .= cleanCsvCall($this->servicesDataArray[$consignmentObj->getServiceId()]['type']) . ",";
        
        $returnString .= "  ,";
        $returnString .= trim(cleanCsvCall($consignmentObj->getReference())) . ",";
        $returnString .= cleanCsvCall($consignmentObj->getConsignmentType()) . ",";
		
        $returnString .= ($consignmentObj->getDateLabelCreated() != '' && $consignmentObj->getDateLabelCreated() > 0 ? formatDate(date("d-m-Y", $consignmentObj->getDateLabelCreated())) : '') . ",";
        $returnString .= ($consignmentObj->getDateScanned() != '' && $consignmentObj->getDateScanned() > 0 ? formatDate(date("d-m-Y", $consignmentObj->getDateScanned())) : '') . ",";
        $returnString .= ($consignmentObj->getDateBooked() != '' && $consignmentObj->getDateBooked() > 0 ? formatDate(date("d-m-Y", $consignmentObj->getDateBooked())) : '') . ",";

        if($consignmentObj->getDateDelivered()!= ''){
             $returnString .= formatDate($consignmentObj->getLastTrackingUpdate())."  ,";
        }else{
              $returnString .= "  ,";
        }
        ...
        $returnString .=  $consignmentObj->getContact(). ",";
        if($action_ext == "DOWNLOAD_CS_CSV"){
            $returnString .= cleanCsvCall($consignmentObj->getAddressLine1()). ",";
            $returnString .= cleanCsvCall($consignmentObj->getAddressLine2()). ",";
            $returnString .= cleanCsvCall($consignmentObj->getAddressLine3()). ",";
        }
        else{
            $returnString .= cleanCsvCall($consignmentObj->getAddressLine1()) . " " . cleanCsvCall($consignmentObj->getAddressLine2()) . " " . cleanCsvCall($consignmentObj->getAddressLine3()) . ",";
        }
        $returnString .= cleanCsvCall($consignmentObj->getCity()) . ",";
 
        if(isset($this->countryDataArray[trim($consignmentObj->getCountryId())])){
        $returnString .= cleanCsvCall( $this->countryDataArray[trim($consignmentObj->getCountryId())]) . ",";
        }else{
           $returnString .= "  ,";   
        }

        $returnString .= cleanCsvCall($consignmentObj->getPostcode()) . ",";
        $returnString .= cleanCsvCall($consignmentObj->getTelephone()) . ",";
        $returnString .= cleanCsvCall($consignmentObj->getDescription()) . ",";
        $returnString .= cleanCsvCall($consignmentObj->getNumberPieces()) . ",";

        if ($consignmentObj->getisDeadWeightChargable() > 0) {
            $returnString .= cleanCsvCall($consignmentObj->getWeight(), 'int') . ",";
        } else {
            $returnString .= cleanCsvCall($consignmentObj->getChargeWeight(), 'int') . ",";
        }
        ...
        $trackingFilterObj = $trackingFilter->getColumnList('carrier_desc,signatory', "", 1 );
        ...
        if(!empty($consignmentObj->getLength())){
             $returnString .= cleanCsvCall($consignmentObj->getLength()) . ",";
        }else{
            
        }
        
        if(!empty($consignmentObj->getWidth())){
             $returnString .= cleanCsvCall($consignmentObj->getWidth()) . ",";
        }else{
            
        }
        
        if(!empty($consignmentObj->getHeight())){
             $returnString .= cleanCsvCall($consignmentObj->getHeight()) . ",";
        }else{
            
        }
   
        $returnString .= cleanCsvCall($consignmentObj->getRoutingCodeEur()) . ",";
	...
	SELECT  
        ...
        if (count($consignmentChagesFilterObj) > 0) {
            foreach ($consignmentChagesFilterObj as $customerChargeData) {
                    ...
                    $returnString .= cleanCsvCall($customerChargeData->getCost()) . " ,";
    ...
    function exportHeader($agentOption = 0, $consignmentChargeTypeCustomerObj = array(), $consignmentChargeTypeAgentObj = array(), $action_ext) {
        $rowToWrite = '';
        if($action_ext == "DOWNLOAD_CS_CSV"){
        ...
        <script type="text/javascript">
            ...
            var DataTableFun = function () {
                var handleDataTable = function () {
                    var datatableurl = "shipment_list_manage_optimize.php?action=consignment_list";
                    grid = new Datatable();
                    grid.init({
            ...
            var DataTableFunParcel = function () {
                var handleDataTableParcel = function () {
                    var datatableparcelurl = "shipment_list_manage_optimize.php?action=parcel_list";
            ...
            $(document).ready(function () {
                ...
                $(".btnExportData").click(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "shipment_list_manage_optimize.php", // your php file name
                        data: form_data,
                        dataType: "json",
                ...
                $("#update_shipnment_status_btn").click(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "shipment_list_manage_optimize.php?action=update_shipnment_status",
                        data: form_data,
                        dataType: "json",
                        success: function (data) {
                    ...
                    if(totalShipmentCount > 25000 ){
                         ...
                         $("#cr_report_text").html('You can only download detail GP report less than 25000 records. For '+totalShipmentCount+' records you can download GP summary report. Please contact to itsupport@oneworldexpress.com for detail report. ') ;
                          ...
                          form_data.push({name: "report_type", value: "detail" });
                            ...
                            $.ajax({
                                type: "POST",
                                url: "shipment_list_manage_optimize.php",
                                data: form_data,
                                dataType: "json",
                                  success: function (data)
                                {
            ...
            function get_carriers(userAccountId) {
                $.ajax({
                    type: "POST",
                    url: "shipment_list_manage_optimize.php?action=get_carrier_services",
            ...
            function updatePricingDetails() {
                ...
                $.ajax({
                    type: "POST",
                    url: "shipment_list_manage_optimize.php?action=save_consignment_charges",
                    data: form_data,
                    success: function (data) {
            ...
            function updateBillingHold() {
                ...
                $.ajax({
                    type: "POST",
                    url: "shipment_list_manage_optimize.php?action=update_billing_hold",
                    data: form_data,
                    dataType: "json",
                    success: function (data) {
                        grid.getDataTable().ajax.reload();
            ...
            function updateManaualPod() {
                ...
                $.ajax({
                    type: "POST",
                    url: "shipment_list_manage_optimize.php?action=update_manaual_pod",
                    data: form_data,
                    dataType: "json",
                    success: function (data) {
                            ...
                            closeOnCancel: true
                                ...
                                function (isConfirm) {
                                    if (isConfirm) {
                                        ...
                                        $.ajax({
                                            type: "POST",
                                            url: "shipment_list_manage_optimize.php?action=save_tariff_consignment_charges",
                                            data: form_data,
                                            dataType: "json",
                                            success: function (data) {
                                                grid.getDataTable().ajax.reload();
                ...
                if (typeof ids !== 'undefined' && ids.length > 0) {
                    ...
                    if (action == "create_manifest") {
                        if (actionProcess == 1) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "shipment_list_manage_optimize.php?action=create_manifest",
                                data: form_data,
                                dataType: "json",
                                success: function (data) {
                        ...
                        $.ajax({
                            type: "POST",
                            url: "shipment_list_manage_optimize.php?action=remove_manifest",
                            data: form_data,
                            dataType: "json",
                            success: function (data) {
                                if (data.status == "success") {
            ...
            function select_dynamic_action() {
                ...
                if (form_action == 're_generate') {
                        ...
                        closeOnCancel: true
                            ...
                            function (isConfirm) {
                                if (isConfirm) {
                                    ...
                                    $.ajax({
                                        type: "POST",
                                        url: "shipment_list_manage_optimize.php?action=update_selected_action_re_generate",
                                        data: form_data,
                                        dataType: "json",
                                        success: function (data) {
                                            grid.getDataTable().ajax.reload();
                    ...
                    $.ajax({
                        type: "POST",
                        url: "shipment_list_manage_optimize.php?action=update_selected_action_close_shipnment",
                        data: form_data,
                        dataType: "json",
                        success: function (data) {
                            grid.getDataTable().ajax.reload();
            ...
            function updateVolWeightCSV() {
                ...
                $.ajax({
                    url: 'shipment_list_manage_optimize.php?action=update_vol_weight',
                    dataType: 'text',
                    cache: false,
                    contentType: false,
                    processData: false,
            ...
            function get_tariff_charges_detail(consignment_id) {
                $.ajax({
                    type: "POST",
                    url: "shipment_list_manage_optimize.php?action=get_tariff_charges",
            ...
            function get_user_detail(userId) {
                $.ajax({
                    type: "POST",
                    url: "shipment_list_manage_optimize.php?action=get_user_details",
                    data: {'user_id': userId},
                    dataType: "html",
                    success: function (data) {
            ...
            function generate_invoice() {
                ...
                $.ajax({
                    url: 'shipment_list_manage_optimize.php?action=add_invoice',
                    data: form_data,
                    type: 'post',
                    dataType: "json",
                    success: function (response) {
                        if (response.status == "success") {
                            ...
                            $.ajax({
                                url: 'shipment_list_manage_optimize.php?action=get_invoice_pdf',
                                data: form_data,
                                type: 'post',
                                dataType: "json",
                                success: function (response) {
...
<label class="label-account">Select Date Range</label>
        ...
        <?php //echo Ddl::generateServiceDDLWithImage('service_id', 'service_id', 'id', ' class="bs-select input-sm form-control form-filter" required="" data-live-search="true" data-show-subtext="true" ', '', '', 'name', 'Select Services'); ?>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="agent" name="agent" class="form-filter select2 form-control" title="Select Agent">
        ...
        <label class="label-account">Show Shipment </label>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="form-group">
                                        ...
                                        <input class="form-control form-filter" id="mawb" name="mawb" type="text" value="" rel="tooltip" data-original-title="MAWB" placeholder="MAWB">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="form-group ">
                                  ...
                                  <label class="label-account">Manifest</label>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group tracking_type_field">
                                        ...
                                        <select id="tracking_type" name="tracking_type" class="form-control select2 form-filter" data-original-title="" title="">
                                            ...
                                            <option value="parcel">Consignment Number</option>
                                        </select>
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="form-group">
                                   ...
                                   <label class="label-account">Invoice Number</label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="form-group">
                                    ...
                                    <label class="label-account">Contact</label>
                                </div>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-sm-3 hidden">
                                <div class="input-group">
                                    <div class="icheck-inline">
                                            ...
                                            <input id="bill_our_account" name="bill_our_account" type="checkbox" class="form-filter icheck" data-checkbox="icheckbox_flat-green" value="1"/> <label class="label-account">Bill Our Account
                                        </label>
                                    </div>
                                </div>
                            </div>
        ...
        <?php if (Permissions::checkFilePermission('legend_for_accounts')) { ?>
                            <div class="row margin-bottom-10">
                                <div class="col-md-3">
                                    <div class="remote-area-shipment legends"></div> Remote Area Shipment
                                </div>
                                <div class="col-md-3">
        ...
        <?php if (Permissions::checkFilePermission('legend_for_customer_Service')) { ?>
                            <div class="row" id="customer_service_legend">
                                <div class="col-md-3">
                                    <div class="remote-area-shipment legends"></div> Dispatched / Not Delivered
                                </div>
                                <div class="col-md-3">
                                    <div class="credit-shipment legends"></div>Delivered
                                </div>
                                <div class="col-md-3">
                        ...
                        <table class="table table-striped table-bordered table-hover table-condensed" id="manage-data-table">                     
                            <thead>
                                <tr role="row" class="heading">
                                    ...
                                    <th>Account <br /> MAWB No. <br /> Date Dispatched</th>
                                    <th>Status <br /> HAWB Number. <br /> Tracking Number</th>
                                    <th>Type <br /> Location</th>
                                    <th>Weight <br /> Vol Weight</th>
                                    <th>Pieces</th>
                                    <th>Value Type</th>
                                    <th>Service Code <br /> Service Name</th>
                                    <th>Remote Area <br /> On Hold</th>
                                ...
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-group1">
                                                <div>
                                                    <input class="form-control purchase_invoice_charges" id="purchase_invoice_reference" name="reference" type="text" value="" rel="tooltip" data-original-title="Reference">
                                                </div>
                                            </div>
                                        </div>
        <?php foreach ($this->consignmentChargesTypesObj as $obj) { ?>
        ...
        ?>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="button" class="btn btn-primary" id="template_vol_weight_btn" >Download Template</button>
                        </div>
                    </div>
                    <!-- /.modal-content --> 
                </div>
