...
class Page extends BasePage {
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2">
                            <div class="form-group  date-date-pic">
                                <input class="form-control" name="date_from" id="date_from" type="text" placeholder="In Date Form" value="<?php echo @$this->date_from; ?>" />
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group  date-date-pic">
                                <input class="form-control" name="date_to" id="date_to" type="text" placeholder="In Date To" value="<?php echo @$this->date_to; ?>" />
                            </div>
                        </div>                        
                        <div class="col-md-2">
                            <div class="form-group  date-date-pic">
                                <input class="form-control" name="out_date_from" id="out_date_from" type="text" placeholder="Out Date Form" value="<?php echo @$this->out_date_from; ?>" />
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="form-group  date-date-pic">
                                <input class="form-control" name="out_date_to" id="out_date_to" type="text" placeholder="Out Date To" value="<?php echo @$this->out_date_to; ?>" />
                            </div>
                        </div>                        
                        <div class="col-md-2">
                            <div class="form-group">
                        ...
                        <table class="table table-striped table-bordered table-advance table-hover">
                            <thead>                                
                                <tr>
                                    ...
                                    <th class="red-back">Out Date</th>
                                    <th class="red-back">Item</th>
                                    <th class="red-back">Remarks</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
    ...
    public function renderHead() {
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function() {
                ...
                $("#out_date_from").datepicker({dateFormat: 'yy-mm-dd', showOn: 'button', buttonImage: '../../app_theme/images/calendar.gif', buttonImageOnly: true});
                $("#out_date_to").datepicker({dateFormat: 'yy-mm-dd', showOn: 'button', buttonImage: '../../app_theme/images/calendar.gif', buttonImageOnly: true});
    ...
    public function init() {
        ...
        if(isset($_POST['func']) && $_POST['func'] == 'getrack_shelfs'){
            ...
            $rack_id = htmlspecialchars($_POST['rack_id']);
        ...
        if (isset($_POST['warehouse_id'])) {
            ...
            $_SESSION['REPORT_STOCK_OUT']['WAREHOUSE_ID'] = $warehouse_idp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['WAREHOUSE_ID'])) {
            $warehouse_idp = $_SESSION['REPORT_STOCK_OUT']['WAREHOUSE_ID'];
        }
        $this->warehouse_id = $warehouse_idp;
        
        if (isset($_POST['rack_id'])) {
            $rack_idp = htmlspecialchars($_POST['rack_id']);
            $_SESSION['REPORT_STOCK_OUT']['RACK_ID'] = $rack_idp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['RACK_ID'])) {
            $rack_idp = $_SESSION['REPORT_STOCK_OUT']['RACK_ID'];
        }
        $this->rack_id = $rack_idp;

        if (isset($_POST['rack_shelf_id'])) {
            $rack_shelf_idp = htmlspecialchars($_POST['rack_shelf_id']);
            $_SESSION['REPORT_STOCK_OUT']['RACK_SHELF_ID'] = $rack_shelf_idp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['RACK_SHELF_ID'])) {
            $rack_shelf_idp = $_SESSION['REPORT_STOCK_OUT']['RACK_SHELF_ID'];
        }
        $this->rack_shelf_id = $rack_shelf_idp;

        if (isset($_POST['goods_name'])) {
            $goods_namep = htmlspecialchars($_POST['goods_name']);
            $_SESSION['REPORT_STOCK_OUT']['GOODS_NAME'] = $goods_namep;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['GOODS_NAME'])) {
            $goods_namep = $_SESSION['REPORT_STOCK_OUT']['GOODS_NAME'];
        }
        $this->goods_name = $goods_namep;

        if (isset($_POST['user'])) {
            $userp = htmlspecialchars($_POST['user']);
            $_SESSION['REPORT_STOCK_OUT']['USER'] = $userp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['USER'])) {
            $userp = $_SESSION['REPORT_STOCK_OUT']['USER'];
        }
        $this->user = $userp;

        if (isset($_POST['customer'])) {
            $customerp = htmlspecialchars($_POST['customer']);
            $_SESSION['REPORT_STOCK_OUT']['CUSTOMER'] = $customerp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['CUSTOMER'])) {
            $customerp = $_SESSION['REPORT_STOCK_OUT']['CUSTOMER'];
        }
        $this->customer = $customerp;
        
        if (isset($_POST['date_from'])) {
            $date_fromp = htmlspecialchars($_POST['date_from']);
            $_SESSION['REPORT_STOCK_OUT']['DATE_FROM'] = $date_fromp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['DATE_FROM'])) {
            $date_fromp = $_SESSION['REPORT_STOCK_OUT']['DATE_FROM'];
        }
        $this->date_from = $date_fromp;

        if (isset($_POST['date_to'])) {
            $date_top = htmlspecialchars($_POST['date_to']);
            $_SESSION['REPORT_STOCK_OUT']['DATE_TO'] = $date_top;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['DATE_TO'])) {
            $date_top = $_SESSION['REPORT_STOCK_OUT']['DATE_TO'];
        }
        $this->date_to = $date_top;
        
        if (isset($_POST['out_date_from'])) {
            $out_date_fromp = htmlspecialchars($_POST['out_date_from']);
            $_SESSION['REPORT_STOCK_OUT']['OUT_DATE_FROM'] = $out_date_fromp;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['OUT_DATE_FROM'])) {
            $out_date_fromp = $_SESSION['REPORT_STOCK_OUT']['OUT_DATE_FROM'];
        }
        $this->out_date_from = $out_date_fromp;

        if (isset($_POST['out_date_to'])) {
            $out_date_top = htmlspecialchars($_POST['out_date_to']);
            $_SESSION['REPORT_STOCK_OUT']['OUT_DATE_TO'] = $out_date_top;
        } else if (isset($_SESSION['REPORT_STOCK_OUT']['OUT_DATE_TO'])) {
            $out_date_top = $_SESSION['REPORT_STOCK_OUT']['OUT_DATE_TO'];
        }
        $this->out_date_to = $out_date_top;
        ...
        if (!empty($this->user)){
            $whereArr[] = "(u1.full_name LIKE '%" .DbAccess3::escape( $this->user) . "%'  OR u1.user_account  LIKE '%" . DbAccess3::escape( $this->user) . "%' OR u2.full_name LIKE '%" . DbAccess3::escape( $this->user) . "%' OR u2.user_account  LIKE '%" . DbAccess3::escape( $this->user) . "%')";
        }        
        ...
        if (!empty($this->out_date_from)) {
            $whereArr[] = "DATE(lrs.out_date) >= '" . $this->out_date_from . "'";
        }
        if (!empty($this->out_date_to)) {
            $whereArr[] = "DATE(lrs.out_date) <= '" . $this->out_date_to . "'";
        }
        if (!empty($whereArr))
        ...
        $sql = "SELECT
                  ...
                  FROM
                    ...
                    JOIN `user` cust
                      ON cust.`id` = lrs.`customer_id`
                  WHERE lrs.`out_date` IS NOT NULL " . $whereStr . "
        ...
        if(isset($_POST['form_action']) && $_POST['form_action'] == 'Export'){
            ...
            $csvContent = "Warehouse,Rack,Shelf,In Date,In By,Out Date,Out By,Item Name,Dimension,Weight,Remarks"."\r\n";
            if(count($reportData) > 0){
                foreach ($reportData as $rpt){
                    $csvContent .= $rpt['warehouse_name'].",".$rpt['title'].",".$rpt['short_title'] ." ".($rpt['shelf_no']+1).",".$rpt['in_date'].",".$rpt['in_by_user'].",".$rpt['out_date'].",".$rpt['out_by_user'].",".$rpt['goods_name'].",".$rpt['dimension'].",".$rpt['weight']."Kg".",".$rpt['description']."\r\n";
                }
            }
            echo $csvContent;
            $uniqueFileName = "report-stock-out"."-".time();
