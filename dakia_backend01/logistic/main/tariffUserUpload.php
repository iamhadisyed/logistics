...
foreach($userAccountOweChina as $account)
{
	...
	//EEURCORRUTRSELEAD 	  
	$ChinaTariff	=	array( 'EEURCORRUTROWECH');

	$tariffAssigned			=	array();
	$queryDefaultTariff		=	"SELECT * FROM tariff_user_mapping WHERE user_account = '".DbAccess3::escape($account)."'";
	...
	foreach($ChinaTariff as $tariffName)
	{
		...
		{
			$subqueryArray[]	=	"('".DbAccess3::escape($tariffName)."', '".DbAccess3::escape($account)."', '2015-07-07 17:00:00')";
			$queryDe;
		}
	}
